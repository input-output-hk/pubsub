#!/usr/bin/env python3
"""M2 transmission unreliability: per-message delivery under iid send loss.
...
in-exponent in the family.  Exact single-defect law (P = p_fail^(r+1)):
    ...
    q_out = (1 - (1-P) RF/(N-1))^(H-1)     (each honest requester, no mu:
                                            the requesters are honest)
...
Usage: python3 sweep_m2_pfail.py [--mc] [--trials T] [--seed SEED]

Default run is closed-form only (fast).  --mc (LONG, ~4 min; never run by
...
from sweep_m2_cost import flood                     # noqa: E402
...
def q_in(P: float) -> float:
    ...
    (adversarial forwarder or lost send)."""
    return q_own(RF, P)


def q_out(P: float) -> float:
    """Per-message out-cut of the publisher: every requester send lost
    (the publisher's own picks fetch, they do not inject)."""
    return q_acc(RF, P)
...
def churn_E(mu_eff: float) -> float:
    ...
    the identity's uncorrected reading (branching form of
    sweep_m2_mu_shift.py)."""
    h = N - int(round(mu_eff * N))
    rho = rho_giant(RF * (1 - mu_eff))
    return h * ((1 - rho) + u_iterate(mu_eff, RF))
...
def flood_pfail(adj, seed_depths, H, rng, p_fail: float, retries: int):
    ...
    At p_fail = 0 this is bit-identical to sweep_m2_cost.flood."""
    depth = [-1] * len(adj)
    parent = [-1] * len(adj)
    dq = deque()
...
def m2_message(g: M3Graph, rng: random.Random, p_fail: float, retries: int):
    """One message from publisher 0 through the lossy pull-relay cascade
    (publisher-only injection: no initiation links in M2)."""
    return flood_pfail(g.adjacency(), [(0, 0)], H, rng, p_fail, retries)
...
def mc_anchor(rng: random.Random, trials: int) -> None:
    ...
    for _ in range(trials):
        g = M3Graph(p, rng)
        adj = g.adjacency()
        ref_sends, ref_max, ref_mean, ref_r = flood(adj, 0, H)
        at, de, ds = m2_message(g, rng, 0.0, 0)
...
def mc_degree_mix(rng: random.Random, graphs: int) -> None:
    ...
    _report_mix("honest requesters (q_out, binomial)",
                pmf_accepted(RF, 40), reqs, graphs, 10)
...
def mc_elevated(rng: random.Random) -> None:
    ...
    for target, T in ((0.10, 400), (0.40, 250)):
        ...
        for _ in range(T):
            g = M3Graph(p, rng)
            _at, _de, ds = m2_message(g, rng, pc, 0)
            miss = H - len(ds)
            missed_tot += miss
            bad += miss > 0
        mc = bad / T
...
def main() -> None:
    ...
    ap.add_argument("--mc", action="store_true",
                    help="loss-injected MC: anchor, degree mixture, elevated "
                         "cells, retry sweep (LONG, ~4 min; never run by CI)")
    ap.add_argument("--trials", type=int, default=40,
                    help="graphs for the anchor / retry cells (default 40/20)")
    ap.add_argument("--seed", type=int, default=20260813)
    args = ap.parse_args()

    print(f"M2 transmission unreliability -- RF = {RF}, "
          f"N = {N}, mu = {MU}, delta = {DELTA:g}")

    # consistency anchor: the p_fail = 0 law matches the isolated-vertex
    # reading of the published coverage law (branching refinements < 0.1 %)
    q_noreq = (1 - RF / (N - 1)) ** (H - 1)
    e0 = H * (q_in(0.0) + q_out(0.0))
    assert math.isclose(e0, H * (M2Params(N=N, k=K, RF=RF).q_pull() + q_noreq),
