#!/usr/bin/env python3
"""M4 transmission unreliability: per-message delivery under iid send loss.
...
with p = p_fail.  Two accounting corrections separate loss from churn:
  ...
  (ii) the isolation event splits per direction: a node misses the message
       when every delivery INTO it is lost; the publisher is muted when
       every send OUT of it is lost.  The two directions of an undirected
       link are independent Bernoullis, so per message
       q_in = q_out = q_dir and eps_msg ~ H q_dir -- the same count as the
       per-epoch law (the out-class loses its H, the in-class splits off
       with its own H-1).
...
single-defect law (P = p_fail^(r+1)):
    ...
    q_dir = E[P^D] * (1 - (1-P) RF/(N-1))^(H-1),  D = honest count among
            the node's RF own picks (both factors per direction)
    eps_msg   = 1 - (1 - q_dir)^H
    E[missed] = (H-1)(2 q_dir - q_dir^2)

(at p_fail = 0 both factors reduce to the published p_isolated).  Bandwidth
counting rule at p_fail > 0: attempted sends keep the p_fail = 0 convention
(fire once, every incident honest link except the arrival link, duplicates
included -- a link can carry the message once each way when the first
crossing failed to deliver or was itself a duplicate); the arrival-link
skip spares only the link of the FIRST SUCCESSFUL delivery.  Backs
../properties/transmission_unreliability.md.

Usage: python3 sweep_m4_pfail.py [--mc] [--trials T] [--seed SEED]
...
validates the honest-degree pmf class by class (the loss part is exact
arithmetic); (c) full loss-injected MC at two elevated cells with
P(miss) ~ 0.1 / 0.4 (the law-vs-MC residual here IS the interior effect);
(d) the retry sweep -- attempted-vs-delivered bandwidth accounting and
depth percentiles at p_fail in {0.01, 0.05, 0.1} x r in {0, 1, 2}.
"""
...
def q_dir(P: float) -> float:
    """Per-message one-direction cut of a fixed honest node: every honest
    incident link (own picks and accepted picks) fails to deliver in that
    direction."""
    return q_own(RF, P) * q_acc(RF, P)
...
def churn_E(mu_eff: float) -> float:
    ...
    return M4Params(N=N, k=int(round(mu_eff * N)), RF=RF).E_isolated()
...
def flood_pfail(adj, seed_depths, H, rng, p_fail: float, retries: int):
    ...
    At p_fail = 0 this is bit-identical to sweep_m4_cost.flood.

    On the undirected adjacency a failed A->B crossing does NOT block a
    later B->A send once B is informed via elsewhere (B's arrival link is
    then not the A-B link) -- exactly the asymmetry the interior-effect
    measurement needs."""
    depth = [-1] * len(adj)
    parent = [-1] * len(adj)
    dq = deque()
...
def m4_message(g: M4Graph, rng: random.Random, p_fail: float, retries: int):
    """One message from publisher 0 through the lossy undirected flood."""
...
def pmf_degree(hi: int):
    """pmf (0..hi) of the total honest degree: own picks + accepted picks
    (independent, so the mixture is the convolution; parallel links from
    mutual picks are counted twice, matching the adjacency)."""
    a, b = pmf_chosen(RF), pmf_accepted(RF, hi)
    out = [0.0] * (hi + 1)
...
def mc_anchor(rng: random.Random, trials: int) -> None:
    ...
    for _ in range(trials):
        ...
        at, de, ds = m4_message(g, rng, 0.0, 0)
...
def mc_degree_mix(rng: random.Random, graphs: int) -> None:
    ...
    for _ in range(graphs):
        ...
        for v in range(H):
            ...
            h[d] = h.get(d, 0) + 1
        degs.append(h)
    _report_mix("honest degree (q_dir, hypergeom * binomial)",
                pmf_degree(40), degs, graphs, 12)


def mc_elevated(rng: random.Random) -> None:
    """(c) full loss-injected MC vs the exact law at elevated cells.  For
    M4 the residual is RECORDED, not gated: any surplus of the law over MC
    here is the interior pair-retry effect (finding 2 of the property)."""
    p = M4Params(N=N, k=K, RF=RF)
    ...
    for target, T in ((0.10, 400), (0.40, 250)):
        ...
        for _ in range(T):
            g = M4Graph(p, rng)
            _at, _de, ds = m4_message(g, rng, pc, 0)
            miss = H - len(ds)
            missed_tot += miss
            bad += miss > 0
        mc = bad / T
...
def main() -> None:
    ...
    print(f"M4 transmission unreliability -- RF = {RF}, "
          f"N = {N}, mu = {MU}, delta = {DELTA:g}")

    # consistency anchor: the p_fail = 0 law equals the published closed form
    assert math.isclose(q_dir(0.0), M4Params(N=N, k=K, RF=RF).p_isolated(),
                        rel_tol=1e-9), "p_fail = 0 law anchor broken"
    ...
    print("(4c) loss-injected MC at elevated cells (residual = interior effect)")
    mc_elevated(rng)
