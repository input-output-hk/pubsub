#!/usr/bin/env python3
"""M1 mu-shift robustness: coverage degradation at frozen parameters.

The deployed operating point (F = 25, margin-selected at N = 20000, mu = 0.2;
delta = 1e-4) is frozen while the effective adversarial fraction mu_eff
...
mu_eff (exact strong-connectivity check, as in sim_m1_coverage.py).

Usage: python3 sweep_m1_mu_shift.py [--quick] [--seed SEED]
"""
...
from m1_model import M1Params, sample_bad  # noqa: E402
...
MC_CELLS = [(0.50, 800), (0.55, 500), (0.60, 300)]   # (mu_eff, trials)


def p_bad(mu: float) -> float:
    return M1Params(N=N, k=int(round(mu * N)), F=F).p_bad()
...
def main() -> None:
    ...
    print(f"M1 mu-shift robustness -- frozen F = {F}, N = {N}, "
          f"delta = {DELTA:g}")
    ...
    for mu, T in MC_CELLS:
        pred = p_bad(mu)
        bad = sample_bad(M1Params(N=N, k=int(round(mu * N)), F=F), T, rng)
        mc = bad / T
