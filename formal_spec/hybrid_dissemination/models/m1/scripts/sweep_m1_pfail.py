#!/usr/bin/env python3
"""M1 transmission unreliability: per-message delivery under iid send loss.
...
For M1 the in-term dominates (reception rides on other nodes' pushes), so
the identity's budget survives almost unchanged -- in fact slightly
TIGHTENED by (i).  Exact single-defect law (P = p_fail^(r+1)):

    q_in  = (1 - (1-P) F/(N-1))^(H-1)      (each honest pusher, no mu: the
                                            pushers are honest by construction)
    q_out = E[P^D],  D = honest picks among the publisher's F targets
...
Usage: python3 sweep_m1_pfail.py [--mc] [--trials T] [--seed SEED]
...
PUB_MSGS, PUB_HOPS_FULL, PUB_HOPS_MEAN = 319_974, 4.92, 3.56
...
def q_in(P: float) -> float:
    """Per-message in-cut of a fixed honest node: no honest pusher's send
    arrives (reception is entirely the accepted side in M1)."""
    return q_acc(F, P)


def q_out(P: float) -> float:
    """Per-message out-cut of the publisher: all F own pushes fail
    (adversarial target or lost send)."""
    return q_own(F, P)
...
def churn_E(mu_eff: float) -> float:
    ...
    return M1Params(N=N, k=int(round(mu_eff * N)), F=F).E_defects()
...
def flood_pfail(adj, seed_depths, H, rng, p_fail: float, retries: int):
    ...
    At p_fail = 0 this is bit-identical to sweep_m1_cost.flood."""
    depth = [-1] * len(adj)
    parent = [-1] * len(adj)
    dq = deque()
...
def m1_message(g: M1Graph, rng: random.Random, p_fail: float, retries: int):
    """One message from publisher 0 through the lossy push cascade."""
...
def mc_anchor(rng: random.Random, trials: int) -> None:
    ...
    for _ in range(trials):
        ...
        at, de, ds = m1_message(g, rng, 0.0, 0)
...
def mc_degree_mix(rng: random.Random, graphs: int) -> None:
    ...
    for _ in range(graphs):
        ...
        for v in range(H):
            ...
            h[d] = h.get(d, 0) + 1
        outs.append(h)
    _report_mix("honest pushers (q_in, binomial)", pmf_accepted(F, 40),
                ins, graphs, 10)
    _report_mix("honest push targets (q_out, hypergeom)", pmf_chosen(F),
                outs, graphs, 12)
...
def mc_elevated(rng: random.Random) -> None:
    ...
    for target, T in ((0.10, 400), (0.40, 250)):
        ...
        for _ in range(T):
            g = M1Graph(p, rng)
            _at, _de, ds = m1_message(g, rng, pc, 0)
            miss = H - len(ds)
            missed_tot += miss
            bad += miss > 0
        mc = bad / T
