#!/usr/bin/env python3
"""M5 mu-shift robustness: coverage degradation at frozen parameters.

The deployed operating point ((k_in, k_out) = (9, 8), chosen at N = 20000,
...
checks at elevated mu_eff (strong-connectivity check, as in
sim_m5_coverage.py).

Usage: python3 sweep_m5_mu_shift.py [--quick] [--seed SEED]
"""
...
from m5_model import M5Params, sample_bad  # noqa: E402
...
def p_bad(mu: float) -> float:
    return M5Params(N=N, k=int(round(mu * N)),
                    k_in=K_IN, k_out=K_OUT).p_bad()
...
def main() -> None:
    ...
    print(f"M5 mu-shift robustness -- frozen (k_in, k_out) = "
          f"({K_IN}, {K_OUT}), N = {N}, delta = {DELTA:g}")
    ...
    for mu, T in MC_CELLS:
        pred = p_bad(mu)
        bad = sample_bad(M5Params(N=N, k=int(round(mu * N)),
                                  k_in=K_IN, k_out=K_OUT), T, rng)
        mc = bad / T
