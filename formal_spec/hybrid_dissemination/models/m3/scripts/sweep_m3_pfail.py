#!/usr/bin/env python3
"""M3 transmission unreliability: per-message delivery under iid send loss.
...
Exact single-defect law used here (P = p_fail^(r+1)):

    q_in  = E[P^D],  D = honest picks among the node's RF pulls
    q_out = (1 - (1-P) RF/(N-1))^(H-1) * E[P^D'],  D' = honest picks among
            the publisher's s-1 initiation targets
    eps_msg   = 1 - (1 - q_out)(1 - q_in)^(H-1)
    E[missed] = (H-1)(q_in + q_out - q_in q_out)

(chosen-side expectations are exact hypergeometric sums; at p_fail = 0 they
reduce to the published coverage-law terms).  Backs
../properties/transmission_unreliability.md.

Usage: python3 sweep_m3_pfail.py [--mc] [--trials T] [--seed SEED]
...
def q_in(P: float) -> float:
    """Per-message in-cut of a fixed honest node: all RF pulls fail
    (initiation links never help reception)."""
    return q_own(RF, P)


def q_out(P: float) -> float:
    """Per-message out-cut of the publisher: every requester send lost AND
    every initiation send fails (adversarial target or lost)."""
    return q_acc(RF, P) * q_own(S - 1, P)
...
def churn_E(mu_eff: float) -> float:
    ...
    return M3Params(N=N, k=int(round(mu_eff * N)), RF=RF, s=S).E_defects()
...
def flood_pfail(adj, seed_depths, H, rng, p_fail: float, retries: int):
    ...
    At p_fail = 0 this is bit-identical to sweep_m3_cost.flood_seeded."""
    depth = [-1] * len(adj)
    parent = [-1] * len(adj)
    dq = deque()
...
def m3_message(g: M3Graph, rng: random.Random, p_fail: float, retries: int):
    """One message from publisher 0: lossy initiation sends (honest targets
    only -- adversarial targets are dead links and never transmissions),
    then the lossy relay cascade.  Returns (attempted, delivered, depths)."""
    seeds = [(0, 0)]
    attempted = delivered = 0
    for t in g.init_targets[0]:
        ...
        if success:
            delivered += 1
            seeds.append((t, 1))
    at, de, ds = flood_pfail(g.adjacency(), seeds, H, rng, p_fail, retries)
    return attempted + at, delivered + de, ds
...
def mc_anchor(rng: random.Random, trials: int) -> None:
    ...
    for _ in range(trials):
        ...
        ref_sends, ref_max, ref_mean, ref_r = flood_seeded(adj, seeds, H)
        ref_sends += len(g.init_targets[0])
        at, de, ds = m3_message(g, rng, 0.0, 0)
...
def mc_degree_mix(rng: random.Random, graphs: int) -> None:
    ...
    for _ in range(graphs):
        ...
        h: dict = {}
        for v in range(H):
            d = sum(1 for f in g.pull_picks[v] if f < H)
            h[d] = h.get(d, 0) + 1
        pulls.append(h)
        h = {}
        for v in range(H):
        ...
        h = {}
        for v in range(H):
            d = len(g.init_targets[v])
            h[d] = h.get(d, 0) + 1
        inits.append(h)
    _report_mix("honest pulls (q_in, hypergeom)", pmf_chosen(RF),
                pulls, graphs, 8)
    _report_mix("honest requesters (q_out, binomial)",
                pmf_accepted(RF, 40), reqs, graphs, 8)
    _report_mix("honest initiation targets (q_out, hypergeom)",
                pmf_chosen(S - 1), inits, graphs, S - 1)
...
def mc_elevated(rng: random.Random) -> None:
    ...
    for target, T in ((0.10, 400), (0.40, 250)):
        ...
        for _ in range(T):
            g = M3Graph(p, rng)
            _at, _de, ds = m3_message(g, rng, pc, 0)
            miss = H - len(ds)
            missed_tot += miss
            bad += miss > 0
        mc = bad / T
...
def main() -> None:
    ...
    ap.add_argument("--mc", action="store_true",
                    help="loss-injected MC: anchor, final-hop, elevated "
    ...
    print(f"M3 transmission unreliability -- (RF, s) = ({RF}, {S}), "
          f"N = {N}, mu = {MU}, delta = {DELTA:g}")

    # consistency anchor: the p_fail = 0 law equals the published closed form
    e0 = H * (q_in(0.0) + q_out(0.0))
    assert math.isclose(e0, M3Params(N=N, k=K, RF=RF, s=S).E_defects(),
