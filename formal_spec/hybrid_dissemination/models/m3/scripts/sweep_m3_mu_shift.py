...
checks at elevated mu_eff (exact every-publisher check, as in
sim_m3_coverage.py).

Usage: python3 sweep_m3_mu_shift.py [--quick] [--seed SEED]
"""
...
MC_CELLS = [(0.35, 800), (0.40, 400), (0.45, 250)]   # (mu_eff, trials)


def p_bad(mu: float) -> float:
    return M3Params(N=N, k=int(round(mu * N)), RF=RF, s=S).p_bad()
...
def main() -> None:
    ...
    print(f"M3 mu-shift robustness -- frozen (RF, s) = ({RF}, {S}), "
          f"N = {N}, delta = {DELTA:g}")
