...
**And that pushes the minimum network size up.** The bucket-size consequence lands directly on the small-network gap named in the previous session. M3 at a replication factor around 37 already carries a documented minimum network size; in the symmetric model, the headroom needed to keep the security guarantee forces a larger bucket, and the larger bucket raises the lower bound on how small a topic can be before the gate stops contributing. Denis suggested the answer is not a single parameter set but several, selected by network size, so that guarantees survive a network transitioning between growth stages. Will will document the gaps for networks below those thresholds for review in a later session.
