...
- Linearizability across topic-filter check, signature-verification step, mutator API, and snapshot getter (003 FR-019 extends 002 FR-015 to cover verification). The receive task processes inbound messages serially (FR-020 single-task model); the verifier MUST be stateless so future verifier impls (real Ed25519 in 011) can offload CPU-heavy verification per FR-020 without changing the trait surface.
