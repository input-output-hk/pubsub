...
- **Three new dependencies**: `rand`, `rand_chacha`, `sha2`. All three live in `[dependencies]` (not `[dev-dependencies]`) because the binary's `main.rs` constructs a `TestVerifier` at the prototype-stage iteration per FR-017, making `crypto::mock` reachable at runtime. None warrants a fresh ADR — the existing ADR 0009 covers the trait-shape decision that motivates the deps; the deps themselves are ubiquitous standard-Rust crates per the Constitution's "Justified dependencies" rule.
