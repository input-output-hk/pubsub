...
- Q: Does the node react to runtime changes in its own subscription-list entry? → A: No — interest set is fixed at startup (read once via `interests_of`); changes to the node's own entry take effect only on restart; dynamic re-derivation deferred to 012.
...
- **Node id absent from the subscription list at startup**: a node whose `node_id` has no entry cannot fabricate interests from config (source-of-truth invariant). When `interests_of(self_id)` returns `None` at startup, the node MUST **fail fast** — treat the missing entry as a hard startup/configuration error rather than running with empty interests. The protocol's faithful wait-and-retry-with-backoff (`joining.md` step 3, where chain lag is real) is deferred to feature 012.
