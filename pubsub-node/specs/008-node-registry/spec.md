...
- **Seam variant → `Event::SubscriptionUpdate(SubscriptionEvent)`** (renamed from the `RegistryUpdate` placeholder anticipated by ADR 0011/CLAUDE.md — needs a heads-up to the 004 author + a one-line update to that comment and the CLAUDE.md SpecKit block when this lands).
- **Node integration targets feature 004's merged pure core.** Crate-internal `pub(crate) fn apply(&mut NodeState, Event) -> Vec<Effect>` (ADR 0011) gains one dispatch line + a named `handle_subscription_update` handler; candidate sets live in crate-internal `NodeState`, exposed via a public `Node` getter; `Effect` is uninhabited so the handler returns an empty `Vec`.
...
**Independent Test**: Construct an `InMemorySubscriptionRegistry` (from a file or programmatically) with nodes across `{T1}`, `{T1, T2}`, `{T2}`. Open `subscribe({T1})`; assert the cold-start burst reports exactly the two `T1` members (each `Joined` carrying `T1`), and neither the `T2`-only node nor `T2`. Then register a fourth node into `{T1}` → one live `Joined`; unregister an original `T1` node → one `Left`.

**Acceptance Scenarios**:

1. **Given** a registry with `A`(`{T1}`), `B`(`{T1,T2}`), `C`(`{T2}`), **When** a consumer opens `subscribe({T1})`, **Then** the watch yields `Joined` for exactly `A` and `B` (each reporting `T1`) as the cold-start burst, and nothing for `C` or `T2`.
2. **Given** an open `subscribe({T1})` whose burst is drained, **When** `D` is registered with `{T1}`, **Then** the watch yields exactly one `Joined { node: D, topics: {T1} }`.
...
5. **Given** `subscribe({T4})` for a topic with no members, **When** the burst completes, **Then** no `Joined` is yielded, and a later registration into `{T4}` yields exactly one live `Joined`.
...
On startup the node looks up **its own entry** in the registry to learn its authoritative topic-interest set, seeds its 002 subscription/filter from that, and `subscribe`s to those topics. A node-owned reader drains the `SubscriptionWatch` and pushes `Event::SubscriptionUpdate(SubscriptionEvent)`; `apply` folds `Joined`/`TopicsChanged`/`Left` into per-topic candidate sets in the crate-internal `NodeState`, excluding the node's own id. The node issues **no** registry writes.
...
**Independent Test** (pure, synchronous, contract §5): Construct `NodeState` for self-id `S`. Feed a scripted `Vec<Event>` of `SubscriptionUpdate` variants — `Joined { A, {T1} }`, `Joined { B, {T1, T2} }`, `Joined { S, {T1} }` (self), `TopicsChanged { A, added: {T2} }`, `Left { B }` — calling `apply` after each. Assert candidate sets: `T1 → {A}`, `T2 → {A}` (self excluded; `B` removed). Assert every `apply` returns an empty `Vec<Effect>`. Separately, assert `interests_of(S)` against a registry seeded `S→{T1}` returns `{T1}`, and that a node configured as `S` but with a registry entry `S→{T1}` acts only on `T1`.

**Acceptance Scenarios**:

1. **Given** a registry entry `S→{T1, T2}`, **When** node `S` starts, **Then** its interest set (and 002 message filter) is `{T1, T2}` — sourced from the registry, regardless of any other value — and it opens `subscribe({T1, T2})`.
2. **Given** a `NodeState` for self-id `S`, **When** `apply` processes `SubscriptionUpdate(Joined { node: S, topics: {T1} })` (own id), **Then** `S` is **not** added to any candidate set (self-exclusion, local).
3. **Given** a state where `A ∈ T1`, **When** `apply` processes `SubscriptionUpdate(TopicsChanged { node: A, added: {T2}, removed: {T1} })`, **Then** `A` is in `T2` and absent from `T1`; the call returns no effects.
4. **Given** a state where `A ∈ T1, T2`, **When** `apply` processes `SubscriptionUpdate(Left { node: A })`, **Then** `A` is absent from every candidate set.
...
- **Self in the stream**: the stream may include the subscriber's own id (the registry is not told who is asking); the node filters its own id locally (US3 AS-2). [NEEDS CLARIFICATION: should the cold-start burst carry an explicit end-of-snapshot boundary marker (e.g. a `SnapshotComplete` `SubscriptionEvent` variant) so a consumer can tell when its candidate set is "warm" before sampling, or is the implicit "burst then deltas" sufficient?]
- **Node id absent from the subscription list at startup**: a node whose `node_id` has no entry cannot fabricate interests from config (source-of-truth invariant). [NEEDS CLARIFICATION: should it (a) error out as a misconfiguration, (b) wait-and-retry until its entry appears — faithful to `joining.md` step 3 — or (c) run with an empty interest set (no topics watched)?]
...
#### Read side (push / subscribe + self-lookup)

- **FR-007**: `SubscriptionRegistry` MUST expose `async fn subscribe(&self, topics: BTreeSet<TopicId>) -> Result<SubscriptionWatch, SubscriptionRegistryError>`. `SubscriptionWatch` MUST mirror `NetworkHandle`: single-consumer, owns the receive half, not `Clone`, ends on drop. On open it MUST replay current members of the watched topics as a `Joined` burst (cold start), then stream live deltas. Event-driven (push), not poll/diff.
- **FR-008**: `SubscriptionRegistry` MUST expose a self-lookup read — `async fn interests_of(&self, node: PeerId) -> Result<Option<BTreeSet<TopicId>>, SubscriptionRegistryError>` — returning a node's registered topic-interest set (`None` if not registered). The node uses this at startup to learn its **own** authoritative interests before it knows which topics to `subscribe` to. This is the enforcement point of the source-of-truth invariant.
- **FR-009**: Events on a single `SubscriptionWatch` MUST be **scoped to the watched topics** (`topics`/`added`/`removed` intersected with the watched set; no events for unwatched topics) and delivered in write-application order, gap-free in-memory. Cross-watch ordering unspecified.
- **FR-010**: The v1 watch channel MUST be **unbounded** (ADR 0007) — no backpressure, no lag/skip event; deferred. `#[non_exhaustive]` `SubscriptionEvent` leaves room for the signal.
...
- **FR-013**: The `Event` enum (feature 004, `src/event.rs`) MUST gain `Event::SubscriptionUpdate(SubscriptionEvent)`. The variant + payload are owned by this feature (contract §3); the `apply` arm is one dispatch line in `pub(crate) fn apply` plus a named `handle_subscription_update(state, update) -> Vec<Effect>` handler (ADR 0011 convention), existing arms untouched. (Renamed from the `RegistryUpdate` placeholder — see Clarifications.)
- **FR-014**: A node-owned producer MUST drain a `SubscriptionWatch` and push one `Event::SubscriptionUpdate(ev)` per `SubscriptionEvent`, registered via `spawn_producer` (node owns the `JoinHandle`, aborts on drop; symmetric with `network_mailbox_loop`). The node MUST consume the registry only through this variant + the FR-008 startup self-lookup; it MUST issue no registry writes.
- **FR-015**: `apply` (via `handle_subscription_update`) MUST fold deltas into a per-topic candidate set of member `PeerId`s in the crate-internal `NodeState`: `Joined` adds, `TopicsChanged` adds/removes, `Left` removes from every set. Pure (no I/O, no `.await`), returns an empty `Vec<Effect>` (`Effect` uninhabited). A public `Node` getter MUST expose a snapshot of the per-topic candidate set (mirroring `received_messages()`); `NodeState` stays `pub(crate)`.
- **FR-016**: The node MUST exclude its own `PeerId` when folding — its id never appears in any candidate set — filtering **locally**; `subscribe` MUST NOT take the subscriber's id.
...
- **FR-021**: The registry module MUST be verifiable **in isolation** (construct/`from_file`, write, `subscribe`, `interests_of`; assert on emitted `SubscriptionEvent`s + cold-start burst). Node integration MUST be testable as a **pure** state-machine exercise (scripted `Vec<Event>` of `SubscriptionUpdate` through `apply`; assert candidate sets + `Vec<Effect>`). The source-of-truth invariant MUST be tested: a node configured as `S` against a registry entry `S→T` acts on `T` regardless of any other configured value. The multi-node topology (US4) is testable over a shared `Arc` (or a shared file) with getter polling to steady state. Tests MUST assert on events/state/snapshots — never on log content (Constitution).

### Key Entities

- **SubscriptionRegistry** (trait): published interface / anti-corruption boundary over the subscription-list source. Methods: `set_interest`, `unregister`, `subscribe`, `interests_of`. Distinct from the future `TopicRegistry`.
...
- **SubscriptionEvent** (`#[non_exhaustive]`): one membership delta — `Joined`/`TopicsChanged`/`Left`, identity + interest only. Payload of `Event::SubscriptionUpdate`.
- **SubscriptionWatch**: single-consumer subscription handle (mirrors `NetworkHandle`) over an unbounded channel; not `Clone`; ends on drop. Cold-start `Joined` burst, then live deltas.
...
- **Event::SubscriptionUpdate**: the single seam the node consumes the registry through.
...
- **SC-006**: The registry's write/`from_file`/subscribe/`interests_of` behaviour is fully verified without the node event loop.
...
- **The seam-variant rename** (`RegistryUpdate` → `SubscriptionUpdate`) needs a heads-up to the 004 author + a one-line update to ADR 0011's illustrative comment and the CLAUDE.md SpecKit block.
