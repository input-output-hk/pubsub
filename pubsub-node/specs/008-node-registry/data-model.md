...
| `entry` | `async fn entry(&self, node: PeerId) -> Result<Option<SubscriptionEntry>, SubscriptionRegistryError>` | Self-lookup: the node's **subscription-list entry** (`topics`, plus future fields), `None` if not registered. The node calls `entry(self_id)?.topics` at startup to learn its **own** authoritative topics (source-of-truth enforcement, ADR 0013). |

## SubscriptionRegistryControl (new, public trait — write surface; `: SubscriptionRegistry`)

The **operator/test write surface**, a separate trait extending the read trait. The node never depends on it; `InMemorySubscriptionRegistry` implements it, and test/operator-sim code holds the concrete type (or `Arc<dyn SubscriptionRegistryControl>`) to drive churn. Keeping these off `SubscriptionRegistry` leaves the node-facing domain interface free of write/test signatures and reflects that the real chain reader (012) has no write method (on-chain writes are transactions).
...
| `topics` | `BTreeSet<TopicId>` | its topic set |
...
subscription-list.toml ──from_file──► InMemorySubscriptionRegistry  (shared via Arc across nodes)
            ...
            operator/test set_topics ───┘          │ (cold-start Joined burst, then live deltas)
                                                      ▼
...
- **Source-of-truth**: a node's effective topics equal its subscription-list entry; config cannot widen them (SC-007). Absent entry at startup ⇒ fail fast.
