...
8. **`subscribe`/`unsubscribe` stay sync** (ADR 0012) — unchanged by this feature; the node's interest set is fixed at startup (spec Clarifications), so runtime self-interest changes are out of scope (deferred to 012).
