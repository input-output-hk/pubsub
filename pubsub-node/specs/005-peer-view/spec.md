...
- **Candidate ordering**: candidates are supplied to selection in a canonical (sorted) order, so the seeded PRNG sample is fully determined by (seed, identity, topic, candidate set) and never depends on incidental data-structure iteration order. There is no ranking and therefore no tie-break to resolve.
...
- **FR-008**: Selection MUST be fully determined by the seed, node identity, topic, and the candidate set, with no residual nondeterminism from data-structure iteration order. Candidates MUST be supplied to the sampler in a canonical order (an ordered structure), so a fixed-algorithm PRNG yields the same subset every run — there is no ranking and hence no tie-break to resolve.
...
- **Seeded PRNG sampling over ordered inputs, no persistent generator state.** Selection samples the candidates with a per-call-seeded pseudo-random generator (a fixed algorithm, so reproducible across machines) fed a **canonically ordered** candidate set (the ordered structures of FR-017). The generator is re-seeded from (seed, node identity, topic) on each call — no generator state persists across calls or is drawn ambiently during a transition. A one-time hash derives the generator seed only; peers are picked by the generator, not ranked by a hash.
- **Per-network seed, per-node derivation.** One seed governs the run; per-node diversity comes from folding the node identity into the generator seed.
