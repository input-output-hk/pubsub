...
> **Redesign note (2026-07-02).** After review with the formal engineers (Ezequiel, Denis), the mechanism moved from *seeded pseudo-random sampling* to the **bucketed-pull** model (`docs/extensions/bucketed-pull.md`): an upstream edge exists iff a **verifiable per-round hash-bucket predicate** holds, so the acceptor independently verifies a request and an adversary cannot exhaust a victim's serving slots by spamming. Per-topic bucket count `B` scales with the topic's size against a **fixed target (connection) degree `target_degree`**; acceptance caps downstream at `OC = ⌈target_degree + c·√target_degree⌉`. Fan-out stays `ForwardToAll`. The original seeded/degree-parameter requirements are superseded by the Functional Requirements below; the pivot is recorded under Clarifications → Session 2026-07-02.
...
- **Selection (dial side).** For a joined topic `T` at the current interval `I`, node `D` dials candidate `U` **iff** the bucket predicate holds:
  `H(genesis, T, D, U, I) mod B == 0`, where `B = max(1, round(|candidates_T| / target_degree))`.
  `target_degree` is a **fixed** configured constant — the target (connection) degree — so the expected out-degree per topic is ≈ `target_degree` regardless of topic size, and small topics (where `B` floors to 1) connect to **all** candidates automatically. (`target_degree` corresponds to the model's `RF` in `docs/extensions/bucketed-pull.md`.)
- **Acceptance (inbound side).** On a verified `Request` from `D` on `T`, acceptor `U` accepts iff **(a)** the same predicate `H(genesis, T, D, U, I) mod B == 0` holds (the request is legitimate this interval — verified by recomputing one hash), **(b)** `T` is registered, **(c)** `D` and `U` share interest in `T` (both members), and **(d)** `U` holds fewer than `OC = ⌈target_degree + c·√target_degree⌉` downstream on `T`. Otherwise it rejects.
...
**Small topics.** No special case: `B = max(1, round(|candidates_T| / target_degree))` floors at 1, and `mod 1 == 0` is always true, so a topic with ≤ ~`target_degree` candidates connects to all of them (the graceful degradation `bucketed-pull.md` §Small-topic regime describes). A fixed `target_degree` (Denis's conservative option) makes this automatic and avoids the `ln`-based degeneracy of a size-derived degree.
...
- Q: Degree control? → A: **Fixed `target_degree`** (Denis's conservative option — sized for the expected max N, swept by the framework). `B = max(1, round(|candidates_T| / target_degree))` ⇒ expected out-degree ≈ `target_degree`. **Small topics handled automatically** by `B=1` (connect-to-all); no `ln`-degeneracy, no network-size estimation.
- Q: Accept cap + scope? → A: `OC = ⌈target_degree + c·√target_degree⌉` (variance buffer, `c` default 3, configurable), applied **per topic**.
...
- Q: Names? → A: `HashGatedConnection` (dial), `VerifiableBoundedAcceptance` (accept), `ForwardToAll` (fan-out); shared predicate `strategies::edge::is_valid_edge`; event `Heartbeat`. *(Superseded by Session 2026-07-07: the acceptance policy is renamed `HashGatedBoundedAcceptance` / `hash-gated-bounded`.)*

### Session 2026-07-07 (epoch/round split + decomposed baselines — ADR 0031)

- Q: One interval for both dial cadence and randomness? → A: **No — split** (agreed with Ezequiel). `Event::Heartbeat` becomes the parameterless dial tick (the **round**); a new `Event::Epoch { nonce }` folds the **epoch nonce** — the randomness context the edge predicate hashes. Repeated heartbeats within an epoch re-dial the same expected set (the retry primitive); epoch advancement is the driver's job, decoupled from dialing.
- Q: Predicate inputs? → A: `H(nonce, requester, candidate, topic) mod B == 0` — the `(genesis, interval)` pair collapses into the single epoch nonce; `--genesis` is the *initial* nonce (node state, not a strategy field). The nonce is designed to arrive from an externally observable event (block hash) when the beacon layer lands.
- Q: Acceptance naming? → A: Aligned with the dial seam: `verifiable-bounded` → **`hash-gated-bounded`** (`HashGatedBoundedAcceptance`).
- Q: Decomposed acceptance? → A: **Implemented now** as the empirical one-dimensional baselines: `bounded` (cap only) and `hash-gated` (gate only) beside `accept-from-all` and the compound. Four concrete structs sharing an `admit_prelude` (membership → idempotent re-Accept), no combinator framework.
- Q: Fail-open admission race (review finding)? → A: Inbound `Request`s are **gated on `Synced`** (silent drop, `not_synced`) — a partially-folded candidate view must not feed B derivation. Scope and residual risk recorded in ADR 0031.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Verifiable hash-gated upstream selection (Priority: P1)

A node forms its per-topic upstream edges from the bucket predicate over `(genesis, T, self, candidate, interval)`, giving a bounded partial topology whose expected degree is ≈ `target_degree`. Re-running with the same genesis, membership, and interval reproduces an identical edge set.

**Independent Test**: With more candidates than `target_degree`, capture a node's upstreams for a genesis + interval; recompute — identical. Sweep candidate-set size and confirm expected degree tracks `target_degree`. With ≤ ~`target_degree` candidates, all are selected (`B=1`).

**Acceptance Scenarios**:

1. **Given** more candidates than `target_degree`, **When** selection runs, **Then** exactly the candidates with `H(genesis, T, self, candidate, interval) mod B == 0` are selected (`B = max(1, round(|candidates_T|/target_degree))`), expected count ≈ `target_degree`.
2. **Given** ≤ ~`target_degree` candidates on the topic, **When** selection runs, **Then** `B = 1` and all candidates are selected (connect-to-all).
...
- **Small topic** (`|candidates_T| ≤ ~target_degree`): `B=1` ⇒ every candidate selected/accepted (connect-to-all) — no special case, and (per the doc) bucketing gives no meaningful security here; delivery leans on relay/local tiers (out of scope).
- **No candidates** (only self on the topic): no edges.
- **Predicate false for a candidate this interval**: simply not an upstream — expected (degree is probabilistic around `target_degree`).
...
- **FR-001**: The dial-side policy MUST select, per joined topic, exactly the candidates for which `H(nonce, self_id, candidate_id, topic) mod B == 0` (the current epoch nonce; Session 2026-07-07), where `B = max(1, round(|candidates_on_topic| / target_degree))`.
- **FR-002**: The edge predicate MUST be a pure, deterministic, verifiable function of the public values `(nonce, requester_id, candidate_id, topic)` (the epoch nonce; Session 2026-07-07), ordered `(requester, candidate)` (directional); it MUST use a fixed hash (SHA-256 over a canonical length-prefixed encoding — not `DefaultHasher`), draw no ambient randomness, depend on no wall-clock, and reproduce identically across machines.
- **FR-003**: `target_degree` (the fixed target connection degree) MUST be a **fixed** configured constant applied uniformly for the run (not derived from network size), so expected out-degree per topic ≈ `target_degree` and small topics connect to all candidates automatically via `B=1`.
...
- **FR-006**: Selection and acceptance MUST use the **current epoch nonce**, folded from `Event::Epoch { nonce }` (initially the configured genesis); the dial tick is the parameterless `Event::Heartbeat` (Session 2026-07-07). The core MUST NOT advance either by wall-clock; a driver fires both. The node MUST retain the current nonce so the acceptor verifies against it.
- **FR-007**: The inbound acceptance policy (`hash-gated-bounded`) MUST accept a verified `Request` from `requester` on `topic` iff ALL hold: (a) `H(nonce, requester, self_id, topic) mod B == 0`; (b) the topic is registered; (c) requester and acceptor share interest in the topic (both members); (d) the acceptor holds fewer than `OC = ⌈target_degree + c·√target_degree⌉` downstream on the topic (`c` a configured buffer constant, default 3). Otherwise it MUST reject. The one-dimensional baselines `bounded` (only (b)–(d)) and `hash-gated` (only (a)–(c)) MUST also be selectable (Session 2026-07-07).
...
- **FR-009**: On a `Rejected`, the dialer's ONLY action MUST be to remove the matching pending (`AwaitingAccept`) upstream. NO retry, NO back-fill, NO failed-peer state; realized degree MAY settle below `target_degree`. Re-forming connections is deferred to the heartbeat-rotation layer + a future retry strategy family.
...
- **FR-012**: This feature MUST fire the heartbeat **once** (at readiness) and never fire `Epoch` (the nonce stays at the configured genesis). Periodic heartbeats, epoch advancement, and cross-epoch connection **rotation/teardown** are out of scope; `Heartbeat` + `Epoch { nonce }` MUST be shaped so that layer drops in without reshaping the seam (Session 2026-07-07).
- **FR-013**: Topology MUST be observable through state getters/snapshots (not logs) — each node's current upstream and downstream sets — so behaviour (degree ≈ target_degree, verifiability, under-fill, `OC` bound) can be asserted and measured.
- **FR-014**: Selection/acceptance and connection state MUST use deterministic, ordered structures (`BTreeSet`/`BTreeMap`) for order-stable effects/snapshots; the strategy objects MUST be pure and free of hidden state (`target_degree`/`c` are fields, the epoch nonce is a view input; Session 2026-07-07), compatible with the strategies-as-arguments refactor.
- **FR-015**: Strategy construction MUST follow the two-phase pattern (ADR 0028): phase-1 key → kind (default on absent, unknown rejected at parse); phase-2 binds each seam's own parameters (genesis, `target_degree`, `c`) and builds, validating required params via a fallible build; the edge maps one typed error.
...
- **`target_degree` (target connection degree)**: a fixed configured constant — the target expected out-degree per topic.
- **Bucket count `B`**: `max(1, round(|candidates_on_topic| / target_degree))` — derived per topic; `B=1` ⇒ connect-to-all.
- **`OC` (accept cap)**: `⌈target_degree + c·√target_degree⌉` downstream per topic (`c` default 3).
...
- **SC-004**: Expected out-degree per topic tracks `target_degree` across candidate-set sizes (within tolerance); no node exceeds `OC` downstream per topic in 100% of runs.
...
- **SC-006**: Below the small-topic point (`|candidates_T| ≤ ~target_degree`, `B=1`) both seams reproduce connect-to-all / accept-all; selecting the unbounded policies is likewise unchanged (no other code path affected).
...
- **Fixed `target_degree`.** Conservative fixed target (connection) degree (Denis's option 1) — handles small topics automatically (`B=1`), no `ln`-degeneracy, no size estimation. `target_degree` and the `OC` buffer `c` are config (swept by the framework); placeholder defaults `target_degree≈8`, `c=3`.
