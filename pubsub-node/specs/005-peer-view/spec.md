...
- **FR-001**: The dial-side policy MUST select, per joined topic, exactly the candidates for which `H(genesis, self_id, candidate_id, topic, interval) mod B == 0`, where `B = max(1, round(|candidates_on_topic| / target_degree))`.
...
- **FR-007**: The inbound acceptance policy MUST accept a verified `Request` from `requester` on `topic` iff ALL hold: (a) `H(genesis, requester, self_id, topic, interval) mod B == 0`; (b) the topic is registered; (c) requester and acceptor share interest in the topic (both members); (d) the acceptor holds fewer than `OC = ⌈target_degree + c·√target_degree⌉` downstream on the topic (`c` a configured buffer constant, default 3). Otherwise it MUST reject.
...
- **FR-014**: Selection/acceptance and connection state MUST use deterministic, ordered structures (`BTreeSet`/`BTreeMap`) for order-stable effects/snapshots; the strategy objects MUST be pure and free of hidden state (genesis/`target_degree`/`c` are fields, interval is an input), compatible with the strategies-as-arguments refactor.
