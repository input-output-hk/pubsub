...
- Handling a `Rejected` only removes the matching pending upstream; no other state changes, and the peer is neither marked nor re-dialed within 005. The realized upstream degree may consequently settle below `RF`; re-forming connections (retry-to-a-minimum / back-fill) is deferred to a future strategy family + the heartbeat-rotation layer, out of scope here.
