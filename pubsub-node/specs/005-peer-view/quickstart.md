...
```text
let selection  = SeededBoundedSelection { seed, self_id, out_degree };   // dial side
let acceptance = BoundedAcceptance { in_degree };                        // inbound side
...
3. To exercise rejection: drive a node past its `in_degree` with inbound requests; observe the over-capacity `Rejected`. On the dialer, after a `Rejected`, re-invoke `ConnectionSetup` and observe back-fill to the next-ranked candidate.

## Assert via getters/snapshots (never logs)

- **Bound** — `upstream_connections()` ≤ `out_degree`/topic; `downstream_connections()` ≤ `in_degree`/topic (SC-002).
- **Reproducibility** — same seed + membership → identical upstream set on rebuild (SC-001).
- **Variety** — two seeds → differing selections for candidates > out-degree (SC-003).
