...
- [X] T005 Add the deterministic keyed-hash ranking helper (stable SHA-256 over length-prefixed `(seed, self_id, topic, candidate_id)`, lowest-k, `candidate_id` tie-break) in `pubsub-node/src/connection.rs` — refactor-agnostic
