...
|-----|-----------|-----------|-------|
| FR-001 predicate selection, degree ≈ target_degree | ✅ | T007, T008 | unit + integration; `bucket_count` |
| FR-002 pure/verifiable predicate | ✅ | T005 (edge unit), T007, T008 | SHA-256, not `DefaultHasher`; order-independent |
| FR-003 fixed target_degree; small-topic connect-to-all | ✅ | T007 (`small_topic`/`out_degree_tracks_target_degree`), T012 | `B=1` floor |
...
| FR-014 ordered structures + pure strategies | ✅ | T005/T006 (`BTreeSet`/`BTreeMap`), T009 | genesis/target_degree/c fields, interval input |
...
| SC-004 degree tracks target_degree; no node exceeds `OC` | ✅ | T007, T008, T012 | |
...
- **I3 — resolved.** All derived docs re-mapped from the superseded seeded-PRNG mechanism to the verifiable hash-gated (bucketed-pull) design and to spec.md's FR-001..016 / SC-001..007 set (this pass): `strategies::edge`, `HashGatedConnection`, `VerifiableBoundedAcceptance`, `Heartbeat { interval }`, `--genesis`/`--target-degree`/`--cap-buffer` throughout.
