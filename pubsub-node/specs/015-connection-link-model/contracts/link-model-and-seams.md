...
2. Under the M3 configuration (`forward-to-all`/`role-scoped` + `owner-only`), a `Publisher` link never carries an `Origin::Peer` forward (send side, FR-005) and never admits one (receive side, publisher binding — drop cause `relay_over_publish_link`). The M5 configuration (`role-agnostic` + `any-verified`) waives both halves by design (FR-015).
