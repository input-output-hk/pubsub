...
- **FR-013**: The fan-out seam MUST be a selectable strategy kind — `forward-to-all` (default; the M3 semantics), `role-scoped` (strict-partition experiment variant), and `role-agnostic` (the M5 semantics — no link-role distinction: every held message over relay-in ∪ publish-out, any origin, minus the arrival link) — reading the link store's sink facets, so dissemination models are configurations rather than code changes (ADR 0034/0035/0036).
...
- **Link**: one logical connection identified by `(peer, topic, role, direction)` — stored flow-oriented as source/sink facets in nested topic → peer maps (ADR 0036; borrowed-key lookups, per-topic subtree walks), with role × direction as the stable view — carrying (for outbound) the lifecycle `state`. A `Relay` and a `Publisher` link between the same pair coexist as independent entries, as do the two directions of one role. Replaces `UpstreamState` and the flat downstream set.
