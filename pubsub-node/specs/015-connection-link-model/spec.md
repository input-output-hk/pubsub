...
- **FR-005**: Fan-out MUST be **origin-aware**: the seam MUST receive whether the message is `Origin::Local` or `Origin::Peer(...)`, and no kind may ever include a `Publisher`-role link for an `Origin::Peer` message (publishing links do not relay). Under the default `forward-to-all` kind, `Publisher`-role links are included **only** for `Origin::Local` messages while `Relay`-role links are included for both origins; the `role-scoped` kind partitions strictly (FR-013).
