...
- **FR-005**: Fan-out MUST be **origin-aware**: the seam MUST receive whether the message is `Origin::Local` or `Origin::Peer(...)` and MUST include `Publisher`-role links **only** for `Origin::Local` messages, while `Relay`-role links are included for both origins.
- **FR-006**: Fan-out over `Relay` links MUST remain identical to `ForwardToAll` (every `Relay` link on the topic, minus the split-horizon exclusion), so relay-only dissemination is unchanged.
...
- **Link**: one logical connection keyed by `(peer, topic, role)`, carrying `direction` and (for outbound) lifecycle `state`. A `Relay` and a `Publisher` link between the same pair coexist as independent entries. Replaces `UpstreamState` and the flat downstream set.
