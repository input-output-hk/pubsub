...
### Design review 2026-06-11 (validation placement + formal-model fidelity)

- Q: Should topic-validity be enforced in the **node** (fold the topic-registry stream into `NodeState` and intersect) or in the **subscription registry** (validate against the topic registry internally and sanitize invalid topics out of the events it emits)? → A: **In the node.** The node already consumes the topic registry directly for **publisher authorization** (FR-015) — that data lives only in the topic registry — so the node↔topic-registry stream is non-optional; topic-validity is then a free intersection on the same `registered_topics` projection. Moving validity into the subscription registry would not remove the node's topic-registry dependency (authorization still needs it), would couple two **independent** on-chain artifacts (the subscription-list contract does not reference the topic registry — it is not even formally modelled), would break the clean per-artifact 012 reader swap, and would bypass the event-queue/pure-`apply` fold (the less event-native path). The node never *acts* on a phantom topic regardless — it is excluded from the effective accept-filter and its traffic is dropped + logged (FR-014). Confirms the FR-011..016 design. (Global vs topic-scoped watch stays as research D2: global, for the mock.)
- Q: How should the registry **entry** be defined against the Quint model (`formal_spec/topic_registry/`), and is the node-facing projection complete? → A: The Quint `Topic` record carries `{ name, owners, admins, publishers (Set[PublicKey], empty ⇒ open), replicationFactor, retentionPeriod, alive }`, all identities `PublicKey`; the authorization matrix gates writes by **owner** (delete, owner/admin grants) or **owner-or-admin** (publisher edits, R/T). The **node consumes none of** owners/admins/R/T — only registered-topics + authorized-publishers — so the node-facing projection stays **publishers-only** (forward-compatible-interface standard: no consumer ⇒ don't carry them). The mock's write surface (`set_topic`/`remove_topic`) stays **permissionless** (operator/test actions); **owner/admin-gated governance is deferred to 012** (FR-017). Authorized publishers are keyed by `PublicKey`, matching the model and the message `publisher_id` — and in the protocol this is the **same identity** the subscription list uses (node pubkey); the mock's `PeerId`(string) vs `PublicKey`(bytes) split is a pre-011 artifact, recorded as IMPLEMENTATION_NOTES **N-009** (identity unification at 011, Principle IV). The model's numeric `TopicID` + `name` collapse to the crate's string `TopicId` in the mock (the 012 reader maps).
...
- **FR-004**: `InMemoryTopicRegistry` MUST provide a `from_file(path)` constructor that loads a **topic-registry file** — read **only** by the topic-registry module, **separate** from 008's subscription-list file — mapping each topic id to its authorized-publisher key set. The format MUST follow the crate's existing TOML config convention: `[[topic]]` tables with `id` and an optional `publishers` list of public keys (an absent or empty `publishers` list means an open topic) — i.e. **only the registered topics and their authorized publishers**, the sole topic-registry facts the prototype's node consumes. The mock file does **not** represent the on-chain contract's governance fields (`owners`, `admins`, `replication_factor`, `retention_period`) — those are 012's domain — so strict unknown-field rejection (per the 001 config policy) applies **uniformly**: any field outside `id`/`publishers` is a load error, with no accepted-but-ignored fields. (This keeps the mock minimal; the mock file is our own config, not a faithful on-chain dump, so it need not tolerate fields the node never reads.) The loaded entries are the registry's initial set of legitimate topics.
...
- **Authorized publishers are keyed by `PublicKey`**, matching the message `publisher_id` (`PublisherId` wraps `PublicKey`) and the formal spec's `publishers: Set[PublicKey]`. Endpoint/identity binding beyond the public key is out of scope. In the protocol this is the **same identity space** the subscription list uses (node pubkey); the current mock's `PeerId` (opaque string) vs `PublicKey` (bytes) split is a pre-011 artifact that unifies when identity becomes pubkey-derived — recorded as **IMPLEMENTATION_NOTES N-009** (Principle IV). The node-facing projection carries **only** registered-topics + authorized-publishers; the formal model's governance fields (`owners`, `admins`, `replicationFactor`, `retentionPeriod`, `alive`) have no node consumer and are deferred to 012 (FR-017), and the mock's write surface stays permissionless (no owner/admin gating).
