...
- **Fresh Claude session per `/speckit-specify`.** Long sessions risk context compression; per-feature sessions keep the agent's attention focused.
- **Stay in one session for meta-work** (brainstorming, prioritizing, roadmap updates) — context continuity helps there. Restart for the per-feature mechanical work.
