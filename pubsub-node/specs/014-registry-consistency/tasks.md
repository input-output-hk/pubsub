...
> **Status: implemented (rebased onto merged 004-connections, 2026-06-17).** All tasks below are done. 004 merged to `main` mid-implementation, expanding scope — the extra work folded into the same tasks: T004's cascade also clears `upstream`/`downstream` (FR-010 flip); a new dial trigger (T005) replaces the removed `connection_setup_delay` timer; a cross-stream readiness gate (T005) orders the topic burst before membership; 004's S7 test was reworked to the rejection behaviour and the 013 SC-004 tests retired. **Superseded 2026-06-17 (b):** T005's dial trigger + readiness gate are now realised by a single `registry_indexer_loop` (drains topic burst then membership burst, then pushes `Event::ConnectionSetup`; per-stream `SnapshotComplete` markers are reader-consumed delimiters, both fold arms no-ops). **Further superseded 2026-06-18 (snapshot reshape):** `watch()` returns `(snapshot, live)`, both `SnapshotComplete` variants removed, single `Event::Synced` readiness + `Syncing`/`Synced` lifecycle (ADR 0020 Amendment 2026-06-18). Full gate green (`fmt`, `clippy -D warnings`, all test binaries, doctests). See spec Clarifications 2026-06-17 + 2026-06-17 (b) + 2026-06-18 + ADR 0020 amendments.
