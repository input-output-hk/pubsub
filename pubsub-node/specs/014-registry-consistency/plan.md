...
> **Rebased onto merged 004-connections (2026-06-17).** 004 merged to `main` mid-implementation; this plan was authored against pre-004 `main`. The scope expanded on rebase (see spec Clarifications 2026-06-17 + ADR 0020 amendment): (a) the `Removed` cascade now also clears `upstream`/`downstream` (FR-010 flipped from "deferred" to in-scope); (b) a new dial trigger replaces 004's removed `connection_setup_delay` timer; (c) a cross-stream readiness gate orders the topic burst before membership; (d) S7/N-015 is resolved. **Superseded 2026-06-17 (b):** (b)+(c) are now realised by a single `registry_indexer_loop` (drains topic burst then membership burst, then pushes `Event::ConnectionSetup` as the single dial trigger; the per-stream `SnapshotComplete` markers are reader-consumed delimiters, both fold arms no-ops). The Design Notes / Source-tree below describe the pre-rebase shape; the as-built deltas are in ADR 0020's amendments (2026-06-17, 2026-06-17 (b), and **2026-06-18 — snapshot reshape**: `watch()` returns `(snapshot, live)`, both `SnapshotComplete` variants removed, single `Event::Synced` readiness + `Syncing`/`Synced` lifecycle).
...
- **Cross-stream readiness gate**: because strict drop evaluates each subscription against the *current* registered set, the node warms its registered-topics projection from the topic-registry cold-start burst **before** it begins folding membership. Mechanism: a `TopicRegistryEvent::SnapshotComplete` marker terminating the cold-start burst; `Node::new` drains the topic watch up to that marker (seeding the projection) before spawning the membership reader. This un-defers a minimal slice of the deferred "registry synchronization complete" signal and is the cross-stream ordering the chain follower provides in production. **⚠ Superseded 2026-06-17 (b) — single-indexer collapse (ADR 0020 amendment; spec Clarifications Session 2026-06-17 (b)):** the two readers + oneshot are replaced by one `registry_indexer_loop` that drains the topic burst then the membership burst (ordering intrinsic, no oneshot) and pushes one `Event::ConnectionSetup`. The per-stream `SnapshotComplete` markers become reader-consumed delimiters (both fold arms no-ops); the dial is `Event::ConnectionSetup`, not the membership marker. **Further superseded 2026-06-18:** the watch is snapshot-reshaped (`(snapshot, live)`), both `SnapshotComplete` variants are removed, and the readiness signal is a single `Event::Synced` driving the `Syncing → Synced` lifecycle (ADR 0020 Amendment 2026-06-18). The mechanism descriptions below in this plan reflect the pre-collapse shape.
