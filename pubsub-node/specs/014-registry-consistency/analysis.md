...
## Session 4 — 2026-06-18 (snapshot-reshaped watch + `Synced` lifecycle)

Follow-on maintainer question: *why two `SnapshotComplete` markers at all?* The markers only existed to delimit each watch's streamed initial burst; the node cares about one thing — being synced. **Decision: reshape the watch contract** so each `watch()` returns `(snapshot, live-watch)` and **remove both `SnapshotComplete` variants**; the indexer folds the topic snapshot then the membership snapshot, then pushes a single new **`Event::Synced`**, which drives an explicit `Syncing → Synced` lifecycle (`NodeState.synced`, `Node::is_synced()`) that establishes connections idempotently. `Event::ConnectionSetup` is retained as the dial action. This supersedes Session 3 (no markers remain; `Synced` replaces the reused `ConnectionSetup` as the readiness signal). Verify-against-code: snapshot-return on both registries (unit tests assert the snapshot, live deltas on the watch), no `SnapshotComplete` anywhere in `src/`, `handle_synced` rising-edge + idempotent, `is_synced()` getter, autonomous-establishment integration test unchanged. Public surface deltas (watch signature, removed variants, `Event::Synced`, `Node::is_synced`) recorded in ADR 0020 (Amendment 2026-06-18), contract §A, data-model §4/§6. Full gate green. **GO**, 0 open findings.
