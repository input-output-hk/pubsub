...
| `cargo build` fails with `Message::Ping(N)` errors after pulling 002 | Call sites still use 001's enum shape. Update to `Message::ping(topic, n)` (the convenience constructor is mandatory per the 002 contract), or to `Message { topic, payload: MessagePayload::Ping(N) }` if the file already has `MessagePayload` in scope. |
