...
- This feature is, like 004, **not parity-preserving** at the integration level: dissemination suites are reworked to assert forwarding. Receive-path unit tests with empty downstream sets are unaffected beyond the shared constructor gaining the fan-out-strategy argument; connection-lifecycle integration suites use the public `ForwardToAll` (verified green — fan-out does not perturb their assertions); there is no test-only no-op strategy (a `#[cfg(test)]` one would be invisible to integration crates, and empty-downstream covers no-fan-out unit assertions). *(The verbatim Input above mentions a `test_support` no-op; it was scaffolded then removed post-implementation — see analysis.md Session 2 finding A2.)*
