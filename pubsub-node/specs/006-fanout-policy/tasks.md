...
**Ordering hazard (binding)**: receive-path fan-out (US2) in a **cyclic** topology loops forever **without** dedup (US3). Therefore US2's integration tests use **acyclic** topologies only (line/tree), and T009 must verify no pre-existing integration suite forms a payload-forwarding cycle (the 2-node suites are cycle-free by split-horizon — the receiver's only downstream is the deliverer, excluded). The first **cyclic** (full-mesh/triangle) payload test lands in US3 (T012), after the `seen` gate exists. **[T009 correction — analysis.md Session 3 F1]** the multi-node "star" suites (`n_node_graph`, `topic_registry_network`) are NOT cycle-free: all co-members share one topic, so the all-candidates policy builds a full **mesh**, not a star — they are `#[ignore]`-deferred to T015 (rework) / T012 (dedup).
...
- [X] T001 [P] Create `src/fanout.rs`: `FanoutStrategy` trait (sync, pure: `targets(&topic, &downstream, exclude) -> Vec<PeerId>`) and `ForwardToAll` impl; failing-first unit tests for `ForwardToAll` (every downstream on the topic; `exclude` removes that peer; empty/other-topic downstream → empty; assertions sort); declare the module in `src/lib.rs` and re-export `FanoutStrategy` + `ForwardToAll`. (No test-only no-op strategy: a `#[cfg(test)]` one is invisible to integration crates, and empty-downstream already gives unit tests a no-fan-out path — see analysis.md Session 2 A2.)
...
- [X] T003 [US1] Write failing sync state tests in `src/state.rs` for `handle_publish`: valid publish records `Origin::Local` + one `Effect::Send` per downstream on the topic (sorted); no-downstream → recorded, no effects; proxy publish (publisher key ≠ self) accepted; the three drop scenarios (not-subscribed / unauthorized / invalid-signature → no record, no effects, and **no severance**; the `topic_not_registered` guard is defensive-under-014 — unreachable on the publish path given the maintained `subscriptions ⊆ registered_topics` invariant, so it is not unit-tested here); each forward is verbatim (the `Effect::Send` payload equals the published `SignedMessage`) (FR-001..005, FR-007/011/016, US1-AS1..4)
...
- [X] T008 [US2] Wire fan-out into `handle_signed_message` in `src/state.rs` via `record_and_fanout(Origin::Peer(from), Some(&from))`, replacing the bare record from T004 — the receive path now records **and** fans out
- [X] T009 [US2] Integration in `tests/dissemination.rs`: scripted **acyclic** line A→B→C → publish at A → C records via relay only, no B→A echo (US2-AS1..3); added the `tests/common` declarative helpers this needs (a test-harness `ConnectToExplicit` connection strategy — dials a fixed edge set, ignoring candidates — plus a fluent `node(...).topic(t).dials(&[(&hub,&t)] | .dials_nobody()).build()` builder; the all-candidates policy over one shared topic can only build a full mesh, so the line is scripted by pinning each node's dialed edges; reused `establish_upstreams`/`await_*`). **Verified** pre-existing suites: the 2-node suites (`two_node_ping`) and single-downstream/star-of-spokes suites (`topic_filter`, `topic_validity`, `connections`, `candidate_set`) are cycle-free under receive fan-out; the US1 `dissemination` star was reworked to a controlled topology. **Finding (corrects the plan's "star cycle-free" assumption):** `n_node_graph` and `topic_registry_network` establish **full meshes** (all co-members dial each other via the all-candidates policy), which loop under receive fan-out until dedup — they are `#[ignore]`-deferred to T015 (rework) / T012 (the cyclic "exactly once" guarantee). See analysis.md Session 3 ⛳
...
- [X] T011 [US3] Add `seen: HashSet<MessageHash>` to `NodeState` and the dedup gate **inside** `record_and_fanout` in `src/state.rs`: compute `MessageHash::of(&signed.plain)`; if present → drop (`duplicate`), return no effects; else insert, record, fan out — after verification, at the shared record point (both paths gain dedup at once). Implemented with `seen.insert(..)` returning false-if-present as the combined check+insert; the `duplicate` log lives at the shared record point (path-agnostic — self_id/topic/publisher_id, no `from`)
- [X] T012 [US3] Integration in `tests/dissemination.rs`: a triangle of three mutually-connected members (the natural full mesh via `establish_mutual` across the three pairs) → one publishes → each node records the message exactly once and propagation terminates (no unbounded circulation), now safe because dedup exists (US3-AS3, SC-002 full / SC-003 / SC-005). **Sanity-check (per Phase-4 input):** the two #[ignore]-deferred suites were run via `--ignored` (no source change) and confirmed to **terminate** under dedup — `n_node_graph` exit 101 (count assertions now stale: it's a mesh, every node records every message once — rework is T015), `topic_registry_network` now **passes** (could be un-ignored at T015). Left ignored per scope. See analysis.md Session 4 ⛳

## Phase 5: Polish & cross-cutting obligations

- [X] T013 [P] Update `specs/IMPLEMENTATION_NOTES.md`: add the deferral entries D1–D5 (next available N-numbers) — bounded `seen` store (real-impl), pick-k fan-out needing a seeded RNG (ROADMAP 006/007), equivocation detection (012, links N-003), `Message::Signed`→`Dissemination` rename, epochal/periodic re-dialer — each cross-referencing data-model §7. Added as **N-021..N-025** (N-020 was taken by the Synced-dialing note); D5 cross-links N-020, D2 the FanoutStrategy seam, D3 N-003
- [X] T014 [P] Refresh rustdoc in `src/node.rs`, `src/fanout.rs`, `src/received.rs`: document `Node::publish`, the `FanoutStrategy` seam + `ForwardToAll`, and the reshaped `Origin` field in stable operator/library terms (no FR citations). The seam/`Origin` docs were already authored in Phases 1–2; added the dedup/duplicate-suppression behavior to the library surface (`Node::publish`) and the `Origin` distinction to `received_messages`
- [X] T015 Rework the remaining dissemination integration suites (the not-parity-preserving charter): where a node holds downstream and receives a valid payload, assert the resulting forwarding. `connections.rs` verified green under the public `ForwardToAll` (no rework). **`topic_registry_network`**: #[ignore] removed — passes under dedup (each node records the authorized message once via the mesh, drops the unauthorized). **`n_node_graph`**: reworked from the obsolete addressed-`send` isolation suites into a single **controlled star** (hub dials nobody, spokes dial only the hub via the `ConnectToExplicit` builder) asserting publish fan-out coverage at N=4 (SC-001/SC-002); both #[ignore]s + the send-isolation scaffolding removed. `two_node_ping`/`topic_filter`/`topic_validity` needed no change (green through Phases 3–4). **No 006-related #[ignore]s remain.**
- [X] T016 Verify-against-code pass for contracts §5: grep `src/lib.rs` re-exports + module visibility (`FanoutStrategy`/`ForwardToAll`/`Origin` exported; `seen`/`fanout`/the helpers/`handle_publish` crate-internal; `ReceivedDelivery.origin` public; confirm no test-only fan-out strategy exists) and reconcile contracts/quickstart if the code diverged. **Verified — no drift**; every §5 row matches code. Added a §5 note: the "no test-only strategy" claim is fan-out-scoped (the harness `ConnectToExplicit` is a *connection* strategy in `tests/common`, out of scope)
- [X] T017 Final validation: full sweep; `quickstart.md` walked against the real API (code blocks compile-accurate — `Node::new` 9-arg order, `publish`, the `Origin` match, `PlainMessage`/`SignedMessage` construction all current); spec SC-001..006 mapped to the demonstrating tests (analysis.md Session 5). fmt + clippy clean; 108 lib + all integration suites green; 0 ignored ⛳
