...
Technical approach per `research.md` R1–R8 and ADR 0020: all behavior lands in the pure core (`state.rs` — a new `handle_publish` arm, the receive arm extended with dedup + fan-out, a shared `fanout` helper), a new `fanout` module for the strategy seam, and a one-parameter addition to `Node::new` plus the `Node::publish` method on the shell. No new producers, no new `Effect` variant, no new dependencies.
...
- **I. Correctness Over Optimization — ✅** Every behavior traces: spec FR-001..016 + Clarifications; research R1–R8; ADR 0020; data-model flows §2–§6 map transitions to FRs; the deferral catalogue §7 maps every gap to its spec line and follow-up.
...
- **III. Document Structural Decisions as ADRs — ✅** ADR 0020 (fan-out strategy seam + content-hash dedup + `Origin`) authored with this plan, referencing 0018's seam rationale and N-005's content-anchored hash. Tactical choices (names, the `duplicate` cause, module layout) are recorded in research/contracts and exempt.
...
docs/decisions/
└── 0020-fanout-strategy-seam-dedup-and-message-origin.md
```
...
|---|---|---|
| 1 | `FanoutStrategy` sync pure trait + `ForwardToAll`; `Arc<dyn>` on `NodeState` beside `strategy`; new `Node::new` param | R1, ADR 0020 §1, contracts §2/§5 |
| 2 | Shared verbatim `fanout()` helper; `Effect::Send` reused; no new effect variant; split-horizon `exclude` | R6, ADR 0020 §2, contracts §2 |
| 3 | `seen: HashSet<MessageHash>` keyed on `MessageHash::of(&plain)`; unbounded | R2, ADR 0020 §3, data-model §1.3 |
| 4 | Dedup after signature verification, at the record point, both paths | R3, ADR 0020 §3, contracts §3 |
| 5 | `Event::Publish` + named `handle_publish`; receive chain minus connection-gate/severance; proxy allowed | R4, ADR 0020 §4, data-model §2 |
| 6 | `Node::publish(self, SignedMessage) -> ()` fire-and-forget | R4, contracts §1 |
| 7 | `ReceivedDelivery.from` → `origin: Origin { Local, Peer(PeerId) }` (fixes doc drift) | R5, ADR 0020 §5, data-model §1.1 |
