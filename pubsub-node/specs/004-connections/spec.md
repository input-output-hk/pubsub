...
- **Duplicate connection request** (peer re-dialing after abrupt restart): idempotent re-accept — keep the existing downstream entry, re-send acceptance. Revisit when a richer acceptance policy exists.
