...
- Q (post-convergence, user-raised during plan preparation): How is a control message's signature verifiable, and which identity does control handling use? → A: The control message is self-contained: the emitter's identity is carried inside the signed content, so the signature binds emitter, topic, and kind (the same shape by which signed payload messages bind the publisher identity). Verification and all control-message handling — self-check, membership validation, entry keying — use the carried emitter identity; the transport frame's sender is not consulted on the control path, consistent with the no-cross-check rule. The identity's representation is a planning-level decision.
- Convergence re-confirmed (round 5, after the post-convergence emitter decision): 0 new decisions; 3 wording alignments disambiguating "sender" now that the two paths use different identities — control handling reads the carried emitter (FR-015, self-emitter edge case), while the payload-path connection check reads the transport's delivering peer as in prior features, since payload messages carry a publisher identity, not the sender's (FR-016). Round 6 (final): 0 decisions; one completion of FR-011's enumeration (reply addressing uses the carried emitter). Fully converged.
...
- **FR-011**: Control messages MUST be signed by the emitting node, with the emitter's identity carried inside the signed content; nodes therefore acquire a signing capability at construction. Verification of control-message signatures MUST happen inside the transition, alongside all other validation. All control-message handling — the self-check, membership validation, the (peer, topic) identity an entry is recorded under, and the addressing of any reply (an Accepted is sent to the carried emitter) — MUST use the carried emitter identity; the transport frame's sender is not consulted on the control path (consistent with FR-015's no-cross-check rule).
...
- **Control message**: signed, topic-carrying protocol message — Request, Accepted, or Terminated — a new kind alongside the existing signed payload and plain messages. Self-contained for verification: the emitter's identity is carried **inside the signed content**, so the signature binds emitter, topic, and kind together (mirroring how the existing signed payload messages bind the publisher identity).
...
- The transport frame's sender identity is trusted as delivered: the in-memory network stamps frames itself, so a frame cannot misattribute its sender. This is what permits the payload-path connection check to key on the frame (FR-016) while control handling keys on the signed emitter (FR-011); frame tampering enters the threat model only with a real transport, alongside the deferred identity-binding hardening.
