...
- [ ] T006 [P] Extend `src/event.rs` with `Event::ConnectionSetup` and `Event::Shutdown` (doc comments name the two producers and the terminal-marker role per ADRs 0018/0019)
...
- [ ] T015 [US1] Extend `tests/common/mod.rs`: alias-keypair fixtures, establishment preamble (topic-registry `set_topic` + subscription-registry `set_topics` → `await_candidates` → push `Event::ConnectionSetup` via `Node::events()`), `await_connection`/`await_upstream_active`-style helpers
