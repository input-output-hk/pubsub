...
- [ ] T001 Verify baseline green on branch `004-node-event-loop`: run `cargo fmt --check && cargo build && cargo clippy --all-targets && cargo test` at the crate root; confirm all 9 integration files under `tests/` pass. This is the SC-001 baseline — `tests/` is not edited by any subsequent task.
