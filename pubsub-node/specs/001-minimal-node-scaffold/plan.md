...
- **III. Document Structural Decisions as ADRs** — ⚠ **at-risk → mitigated**. Five structural choices are introduced (async runtime, config parser, logging stack, CLI parser, error model). Each gets its own ADR slot listed in `research.md`. Mitigation: ADRs are tracked as deliverables in `/speckit-tasks`, not deferred indefinitely.
...
- **III. Document Structural Decisions as ADRs** — ✅ **pass**. `research.md`'s "ADR slot summary" table lists six ADRs (0001 through 0006) covering every structural choice the plan introduces (async runtime, config loader, logging, CLI parser, error model, receive-task & registration timing). `/speckit-tasks` will materialise each as an ADR-authoring task with a logical-increment commit.
