...
- A send targets an identifier that is not registered on the network: the send is dropped (consistent with the trusted, assumed-up setup) AND a warn-level structured log entry naming the unknown identifier MUST be emitted, per FR-010. The sender does not observe a synchronous error.
