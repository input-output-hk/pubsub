...
- A send targets an identifier that is not registered on the network: the send is dropped (consistent with the trusted, assumed-up setup) AND a warn-level structured log entry naming the unregistered identifier MUST be emitted, per FR-010. The sender does not observe a synchronous error.
...
- **FR-004**: The system MUST support a Ping message that carries an opaque numeric value `N` and MUST allow `N` to be inspected by the receiver. Ping is **one-way (fire-and-forget)**: the sender's send operation completes once the network has accepted the message for delivery (FR-013) without waiting for the receiver to acknowledge or process it; the receiver MUST NOT emit any response message; and successful receipt is verified by inspecting the receiver's observable state per FR-006.
...
- **FR-006**: A receiving node MUST expose a queryable record of every received message (carrying at least the sender's id and the message payload), accessible to the operator or test without inspecting internal state directly. Calls to this queryable surface MUST return a snapshot — a value that is stable for the caller and unaffected by subsequent receptions on the same node — so that assertions over the record do not race with the receive path. The "sender id" recorded MUST be the **logical peer identity** — the value returned by `PeerDescriptor::id()` for the originating peer — not a transport-level identifier (e.g., a network address) and not a field embedded inside the message payload. How a node arrives at that value is iteration-dependent and intentionally unspecified here: in v1 the trusted InMemory network supplies it at delivery time (FR-003, FR-007); future iterations may derive it from cryptographic verification of a signed wire envelope. The recorded type and semantics remain stable across iterations. This record is the normative observability surface against which acceptance scenarios assert. Implementations MAY additionally emit structured log output.
...
- **FR-010**: The InMemory network MUST route a message addressed to a registered identifier to the corresponding peer. Messages addressed to an unregistered identifier MUST be dropped (consistent with fire-and-forget send per FR-004) AND MUST result in a warn-level structured log entry that names the unregistered identifier; the send caller does NOT observe a synchronous error in this case.
...
- **FR-012**: The Node public constructor MUST take (a) the node's own identifier as an explicit argument and (b) its peer set as an already-parsed in-memory value (NOT a filesystem path). Loading and TOML parsing of the peer-set config file are handled by a separate CLI/loader layer that yields the parsed value before the Node is constructed. This keeps identity and peer view independent, isolates I/O failure modes from domain logic, and lets tests build a Node without touching the filesystem. The binary's CLI MUST expose `--self-id` (node identifier), a config-path flag, and a log-level flag controlling the verbosity of the structured logs referenced by FR-006 and FR-010. The log-level flag MUST have a default value at which FR-010's warn-level drop events are operator-visible without explicit configuration; the concrete default is recorded in `contracts/cli.md`. Explicit operator overrides of the log level are honoured as given; a more restrictive level (e.g., `error`) MAY suppress FR-010 warn-drop events from operator-visible output, and this is operator choice — not an FR-010 violation, since FR-010 governs the *emission* level (chosen as `warn` because the event is neither informational nor erroneous), not delivery to a human reader. The `--self-id` value MUST be validated against the same identifier rules that apply to peer-entry ids loaded from the config file (FR-009, FR-001); failure to validate MUST cause startup to fail with a clear, actionable error analogous to US3 AS-2's malformed-config path, and the node MUST NOT start with an invalid or default identifier.
- **FR-013**: On the InMemory network, `send(...).await` MUST resolve once the network has accepted the message for delivery to the addressed recipient (enqueue complete). It is NOT required that the recipient's `received_messages()` record contain the message at the moment `send().await` resolves; the recipient MAY process the message into its observable record subsequently. Acceptance assertions on the receiver's record MUST therefore be expressed via an await/poll-with-timeout primitive supplied by the test harness, NOT by inspecting the record immediately after `send().await`. This decoupling preserves contract compatibility with future networked transports. Under the v1 Trust and Liveness assumptions (no peer failures, all peers up, no adversarial behaviour — see spec §Assumptions), each `send().await` MUST result in exactly one delivery to the addressed recipient — no duplication, no loss. The InMemory variant satisfies this trivially via its `mpsc` queue. A violation is observable in either of two forms: (a) a `received_messages()` snapshot containing two or more `ReceivedDelivery` entries whose `(from, message)` pair matches one that was sent only once (duplication), or (b) a `received_messages()` snapshot that, after `await_delivery` resolves successfully for every sent message, is missing any sent `(from, message)` pair (loss). Tests asserting exactly-once at scale (e.g., SC-005's 100-send loop) MAY check both forms. This guarantee is conditional on the v1 assumption set and is NOT expected to survive into networked iterations: real transports fail, the v1 wire protocol intentionally specifies no acknowledgment and no retry (FR-004 fire-and-forget), and future iterations will define explicit delivery semantics (at-most-once / at-least-once / exactly-once) under their own ADRs and spec amendments.
...
- **SC-004**: A new contributor can read this spec together with the feature directory's `quickstart.md`, walk the resulting source and accompanying test harness, and reproduce the two-node Ping demonstration within one hour. The contributor SHOULD NOT need to consult `plan.md`, `research.md`, `contracts/`, ADRs, or any document outside `specs/001-minimal-node-scaffold/`. Amendments to this spec that affect contributor-facing behaviour (FRs governing the CLI surface, the test names referenced in `quickstart.md`, or the user stories' Independent Tests) MUST land alongside the corresponding `quickstart.md` updates in the same commit; reviewers SHOULD reject a spec amendment that leaves `quickstart.md` outdated.
- **SC-005**: The receiving side records every delivered Ping with the original `N` value intact, verified across at least 100 sends. The 100 `N` values MUST be either a deterministic sequence (e.g., `0..100`) or randomly generated from a recorded seed (the seed value MUST appear in the test source — as a `const` or in a doc-comment — so a future reader can reproduce the run without external context), per Engineering Standards "Reproducible tests and simulations"; ad-hoc system randomness is not acceptable.
