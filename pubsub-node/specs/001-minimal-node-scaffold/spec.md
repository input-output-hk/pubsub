...
- **SC-004**: A new contributor can read this spec together with the feature directory's `quickstart.md`, walk the resulting source and accompanying test harness, and reproduce the two-node Ping demonstration within one hour. The contributor SHOULD NOT need to consult `plan.md`, `research.md`, `contracts/`, ADRs, or any document outside `specs/001-minimal-node-scaffold/`.
- **SC-005**: The receiving side records every delivered Ping with the original `N` value intact, verified across at least 100 sends. The 100 `N` values MUST be either a deterministic sequence (e.g., `0..100`) or randomly generated from a recorded seed, per Engineering Standards "Reproducible tests and simulations"; ad-hoc system randomness is not acceptable.
