...
- [X] T002 Add dependencies to `Cargo.toml` per `plan.md` Technical Context: `tokio = { version = "1", features = ["macros", "rt-multi-thread", "sync", "time", "signal"] }`, `serde = { version = "1", features = ["derive"] }`, `toml = "0.8"`, `tracing = "0.1"`, `tracing-subscriber = { version = "0.3", features = ["env-filter", "json"] }`, `clap = { version = "4", features = ["derive"] }`, `thiserror = "1"`. Add `[dev-dependencies]` block reserved for test-only crates (initially empty). Verify `cargo build` still succeeds.
- [X] T003 [P] Configure Clippy lint level in `Cargo.toml` `[lints]` table (deny `rust_2018_idioms`, warn `clippy::pedantic`, allow `clippy::module_name_repetitions`). Run `cargo clippy --all-targets -- -D warnings` and verify it passes on the empty crate.
...
- [X] T004 [P] Author ADR 0001 in `docs/decisions/0001-async-runtime-tokio.md` from `research.md` §1 (async runtime: tokio).
- [X] T005 [P] Author ADR 0002 in `docs/decisions/0002-toml-via-serde.md` from `research.md` §2 (config via serde + toml).
- [X] T006 [P] Author ADR 0003 in `docs/decisions/0003-logging-via-tracing.md` from `research.md` §3 (structured logging via tracing + tracing-subscriber).
- [X] T007 [P] Author ADR 0004 in `docs/decisions/0004-cli-via-clap.md` from `research.md` §4 (CLI via clap derive).
- [X] T008 [P] Author ADR 0005 in `docs/decisions/0005-typed-errors.md` from `research.md` §5 (typed errors via thiserror; no anyhow in library).
- [X] T009 [P] Author ADR 0006 in `docs/decisions/0006-receive-task-and-registration.md` from `research.md` §6 + §8 (recv-task model conjoined with registration timing).
- [X] T010 [P] Author ADR 0007 in `docs/decisions/0007-network-handle-actor-pattern.md` from `research.md` §12 (NetworkHandle as actor-handle — tx/rx split, channels over callbacks). Re-walk and pin URLs at authoring time per the "Note on URL stability" in research §12.
...
- [X] T019 [US1] Write integration tests in `tests/two_node_ping.rs` covering the three US1 acceptance scenarios. Required tests (each `#[tokio::test]`, each using `mod common;` to access `two_node_fixture()` and `await_delivery`):
