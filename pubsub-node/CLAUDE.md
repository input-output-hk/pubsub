...
**Active feature**: **014 — registry consistency** (branch `014-registry-consistency`, spec dir `specs/014-registry-consistency/`). **Specified + clarified + planned + tasked + analyzed (GO) + implemented** (rebased onto merged 004; full gate green) — see [`specs/014-registry-consistency/plan.md`](specs/014-registry-consistency/plan.md). Elevates 013's read-time `subscriptions ∩ registered_topics` into a **maintained `NodeState` invariant** (`subscriptions`/`candidates ⊆ registered_topics.keys()`) via **strict drop** (membership events for unregistered topics dropped + logged, no declared/pending buffer, no auto-promotion — **013 SC-004 removed**); **defensive registry fold** (create-only-on-`Registered`; no `or_default`); **atomic cascade** on `Removed` (clears subscriptions + candidates + projection **+ `upstream`/`downstream`** in one fold); declarative **`TopicEntry`** (`pub(crate)`; `is_open`/`is_publisher_authorized`; `registered_topics: HashMap<TopicId, TopicEntry>`) replacing the inline open-topic check. **Readiness/dial mechanism** (snapshot reshape, 2026-06-18): each `watch()` returns a **`(snapshot, live-watch)`** pair (`TopicSnapshot`/`MembershipSnapshot`); both `SnapshotComplete` event variants are **removed** (no burst to delimit). A **single `registry_indexer_loop`** folds the topic snapshot before the membership snapshot (ordering intrinsic), then pushes one **`Event::Synced`** — the single readiness signal, which transitions the node `Syncing → Synced` (`NodeState.synced`; `Node::is_synced()`) and establishes connections (`handle_connection_setup`), **replacing the removed `connection_setup_delay` timer**. `Event::ConnectionSetup` is retained as the dial **action** (tests / operator / future epochal re-dial). Models the single chain indexer of a realistic prototype (query state at tip, then subscribe; 012 returns its at-tip snapshot likewise); registries stay separate data artifacts. **Resolves S7/N-015**: an unregistered topic is never subscribed/candidate, so a connection `Request` on it is rejected. New **ADR 0020** (amends 0016; + 2026-06-17, 2026-06-17 (b), 2026-06-18 amendments). Next: PR (push to PR #63).
