...
**Active feature**: **014 — registry consistency** (branch `014-registry-consistency`, spec dir `specs/014-registry-consistency/`). **Specified + clarified + planned + tasked + analyzed (GO) + implemented** (rebased onto merged 004; full gate green) — see [`specs/014-registry-consistency/plan.md`](specs/014-registry-consistency/plan.md). Elevates 013's read-time `subscriptions ∩ registered_topics` into a **maintained `NodeState` invariant** (`subscriptions`/`candidates ⊆ registered_topics.keys()`) via **strict drop** (membership events for unregistered topics dropped + logged, no declared/pending buffer, no auto-promotion — **013 SC-004 removed**); **defensive registry fold** (create-only-on-`Registered`; no `or_default`); **atomic cascade** on `Removed` (clears subscriptions + candidates + projection **+ `upstream`/`downstream`** in one fold); declarative **`TopicEntry`** (`pub(crate)`; `is_open`/`is_publisher_authorized`; `registered_topics: HashMap<TopicId, TopicEntry>`) replacing the inline open-topic check. **Readiness/dial mechanism**: both registry watches end their cold-start burst with a `SnapshotComplete` marker (`TopicRegistryEvent::SnapshotComplete` + new `MembershipEvent::SnapshotComplete`); `Node::new` wires an in-node oneshot (topic reader → membership reader) so the topic projection is warm before membership folds (single FIFO queue orders it); **`MembershipEvent::SnapshotComplete` is the event-driven dial trigger** (runs `handle_connection_setup`), **replacing the removed `connection_setup_delay` timer**. **Resolves S7/N-015**: an unregistered topic is never subscribed/candidate, so a connection `Request` on it is rejected (acceptance consistent with registration). New **ADR 0020** (amends 0016; +2026-06-17 as-built amendment). Next: PR.
