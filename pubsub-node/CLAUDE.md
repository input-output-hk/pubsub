...
**Active feature**: **013 — topic registry** (branch `013-topic-registry`, spec dir `specs/013-topic-registry/`). **Specified + clarified + planned** — see [`specs/013-topic-registry/plan.md`](specs/013-topic-registry/plan.md). The in-memory **topic registry**: which topics legitimately exist + each topic's authorized publisher keys (empty set = open topic), the topic-governance counterpart to 008's subscription list (no shared trait — distinct on-chain artifacts per `../docs/node-lifecycle/README.md`; grounded in `../formal_spec/topic_registry/`). `TopicRegistry` trait (single **global** `watch()`, no scoping arg — cf. 008's node-keyed `watch(node)`) + `TopicRegistryControl` (`set_topic`/`remove_topic`, operator/test) + `InMemoryTopicRegistry` (`new()`/`from_file`); `TopicRegistryEvent` (Registered/PublishersChanged/Removed) / `TopicRegistryWatch` (push, mirrors `MembershipWatch`/ADR 0007). Node integration on 004's pure core: `Event::TopicRegistryUpdate` + `handle_topic_registry_update` folds a `registered_topics: HashMap<TopicId, BTreeSet<PublicKey>>` projection into `NodeState`; effective subscriptions = 008's membership-derived `subscriptions` ∩ registered topics (unregistered subscription topics ignored + logged), computed at accept time; `handle_signed_message` drops unauthorized-publisher messages (`publisher_not_authorized`) before signature verification. `Node::new` gains a third generic registry param `Arc<T: TopicRegistry>` (always enforced — delivery tests must register their topics); new `Node::effective_subscriptions` getter; `PublicKey` gains `Ord` (additive). Interface + integration in **ADR 0016** (`docs/decisions/0016-topic-registry-interface-and-node-integration.md`). Clarify 2026-06-11: publisher-authorization enforcement confirmed in scope (drop). Design review 2026-06-11: validation lives in the node (not the subscription registry); node-facing projection is publishers-only (governance deferred to 012; mock writes permissionless; mock file = `id`+`publishers` only); publisher keys ≡ subscription-list node id at 011 (N-009). Tasks + `/speckit-analyze` (analysis.md) done; rebased onto main (constitution v1.2.0 — declarative test builders). GitHub sub-issue #54 under #36. Next: `/speckit-implement`.
