...
**Active feature**: **004-connections** (branch `004-connections`, spec dir `specs/004-connections/`). **Specified + clarified (nine rounds, converged) + planned + tasked + analyzed (3→1→0)** — see [`specs/004-connections/plan.md`](specs/004-connections/plan.md). Logical per-(peer, topic) connections: upstream (`AwaitingAccept`/`Active`) + downstream sets in `NodeState`; strategy-driven establishment on a setup event (optional one-shot timer, **unset by default**, or public-intake injection; diff rule "dial everything expected that is not Active"); signed control messages (`Message::Connection`, emitter inside the signed content); membership-validated idempotent acceptance; connection-gated receive path with silent signature-only misbehavior severance; consuming `shutdown(self)` with Terminated notices; **`PeerId` reshaped to wrap `PublicKey`** (mock alias rule keeps fixtures/logs readable); deliberately **not parity-preserving** (integration suites reworked with establishment preambles). Artifacts: spec (verbatim Input + Clarifications trajectory incl. the 2026-06-12 post-013 reconciliation session), plan + plan-input.md (verbatim, pre-merge record), research (R1–R10), data-model (state machines + staleness catalog S1–S7), contracts/connection-protocol.md, quickstart, tasks T001–T033, checklists/consistency.md (2→0), analysis.md Sessions 1–3; ADRs **0017** (key-backed identity + signed control messages), **0018** (strategy seam), **0019** (shutdown lifecycle, amends 0012). **Post-013 reconciliation applied 2026-06-12**: ADRs renumbered (013 holds 0016); receive chain enumerates the merged checks (connection → subscription → topic-registered → publisher-authorized → signature; severance stays signature-only); `Node::new` baseline includes the topic-registry parameter; the two 013 suites join the rework list; **connection acceptance validates the membership-derived set only — revisit-flagged** pending the cross-registry event-ordering invariant raised on the 013 PR (spec Clarifications, staleness row S7).
