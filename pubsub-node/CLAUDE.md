...
**Active feature**: **014 — registry consistency** (branch `014-registry-consistency`, spec dir `specs/014-registry-consistency/`). **Specified + clarified + planned + tasked + analyzed (GO) + implemented** (rebased onto merged 004; full gate green) — see [`specs/014-registry-consistency/plan.md`](specs/014-registry-consistency/plan.md). Elevates 013's read-time `subscriptions ∩ registered_topics` into a **maintained `NodeState` invariant** (`subscriptions`/`candidates ⊆ registered_topics.keys()`) via **strict drop** (membership events for unregistered topics dropped + logged, no declared/pending buffer, no auto-promotion — **013 SC-004 removed**); **defensive registry fold** (create-only-on-`Registered`; no `or_default`); **atomic cascade** on `Removed` (clears subscriptions + candidates + projection **+ `upstream`/`downstream`** in one fold); declarative **`TopicEntry`** (`pub(crate)`; `is_open`/`is_publisher_authorized`; `registered_topics: HashMap<TopicId, TopicEntry>`) replacing the inline open-topic check. **Readiness/dial mechanism** (single-indexer collapse, 2026-06-17 (b)): a **single `registry_indexer_loop`** owns both watches and drains the topic cold-start burst before the membership burst (ordering intrinsic — the earlier two-reader + oneshot gate is removed), then pushes the existing **`Event::ConnectionSetup`** — the single event-driven dial trigger (`handle_connection_setup`), **replacing the removed `connection_setup_delay` timer**. The per-stream `SnapshotComplete` markers (`TopicRegistryEvent::SnapshotComplete`, `MembershipEvent::SnapshotComplete`) are retained as reader-consumed stream-replay delimiters — **both fold arms are no-ops** (no public readiness event folds into node state). Models the single chain indexer of a realistic prototype (012 emits the one readiness directly); registries stay separate data artifacts. **Resolves S7/N-015**: an unregistered topic is never subscribed/candidate, so a connection `Request` on it is rejected (acceptance consistent with registration). New **ADR 0020** (amends 0016; + 2026-06-17 and 2026-06-17 (b) amendments). Next: PR (push the readiness-collapse revision to PR #63).
