...
**Active feature**: **005-peer-view** (branch `005-peer-view`, spec dir `specs/005-peer-view/`). **Specified + clarified (sticky failed-set; "rejected" = active over-capacity, no timeout; default seed 0; ordered structures in-PR) + planned + tasked (T001–T026) + analyzed (0 critical)** — see [`specs/005-peer-view/plan.md`](specs/005-peer-view/plan.md); next `/speckit-implement` (critical → TDD). **Scope: bounded strategies + their tests only** — the experiment/testing framework is a SEPARATE later feature. Dial side: new `SeededBoundedSelection { seed, self_id, out_degree }` (signature-stable `ConnectionStrategy`; lowest-`out_degree` candidates by stable SHA-256 of `(seed, self_id, topic, candidate_id)`, tie-break on id; node pre-filters a **sticky** `failed_upstream` set out of the candidate view). Inbound: `ConnectionAcceptanceStrategy` return evolves `bool → AcceptanceDecision{Accept,RejectMembership,RejectOverCapacity}` + sees current downstream; new `BoundedAcceptance { in_degree }`. New `ConnectionAction::Rejected` (acceptor→dialer, over-capacity, **not** misbehaviour, **no timeout/no-response notion**) → dialer removes `AwaitingAccept`, marks peer failed (sticky, no retry this run), increments a rejection counter. **Back-fill = re-invoking the existing `ConnectionSetup`** over the viable set (NO new round/tick event, NO timer). Three edge-parsed run-level params (seed, out-degree, in-degree); unbounded defaults preserved (SC-005). **Coordinated with** (not hard-dependent on) the co-developing architect's determinism/purity refactor (strategies-as-`apply`-arguments, deterministic scheduling, decouple-`ConnectionSetup`-from-`Synced` flag): 005 applies ordered structures (`BTreeSet`/`BTreeMap`) to its own state in-PR, keeps strategies pure, retains the current injection, and migrates later — coordinating to avoid conflicts on shared files (`NodeState`, injection sites). Planned ADRs **0024** (seeded bounded selection + stable digest), **0025** (acceptance-seam return evolution + `Rejected`). **Out of scope**: experiment/testing framework (topology builder, delivery-percentile/latency/propagation metrics), golden nodes (push-based M2), edge/golden mode flag, adversarial/Byzantine, bounded/seeded fan-out, any timeout/no-response re-dial.
