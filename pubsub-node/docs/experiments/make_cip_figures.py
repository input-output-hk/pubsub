...
# ------------------------------------------------------------------ figure 5
def fig_design_family() -> str:
    """The five designs as one family: two primitives, two additions, one merge.

    Structural like fig_architecture, so its content is literal rather than
    drawn from cells.json; it stays in this script so that --check keeps every
    figure under one gate. Three numbered bands, read downward in the order
    Designs evaluated walks the field: the one-directional primitives, the two
    designs that add a second link kind to cover both failure directions, and
    the merge that covers both with one kind. Each card carries the design's
    link glyph, the failure Table 7 leaves open, and the pick budget at its
    proposed configuration; the falling budget, 24 to 9, is the argument the
    prose then makes, and the band edges are the checkable nesting boundaries.
    """
    W, H = 860, 610
    b = []
    quiet = "#8a887e"
    ...
    def band(y, h, n, title):
        # numbered, so Designs evaluated can name which tier it is walking
        b.append(rect(38, y, W - 76, h, SURFACE, GRID, 1.2, rx=8))
        b.append(circle(66, y + 19, 11, INK, INK, 0))
        b.append(text(66, y + 23, str(n), 11.5, SURFACE, "middle", "700"))
        b.append(text(84, y + 24, title, 12.5, INK, weight="600"))

    def head(x1, y1, x2, y2, col, size=6.0):
        # arrow()'s polygon head alone, so a line can carry one at each end
        dx, dy = x2 - x1, y2 - y1
        ln = math.hypot(dx, dy) or 1.0
        ux, uy = dx / ln, dy / ln
        bx, by = x2 - ux * size, y2 - uy * size
        px, py = -uy * size * 0.5, ux * size * 0.5
        return (f'<polygon points="{x2:.1f},{y2:.1f} {bx + px:.1f},{by + py:.1f} '
                f'{bx - px:.1f},{by - py:.1f}" fill="{col}"/>')

    def glyph_arrow(x1, y1, x2, y2, col, t1, t2, w=1.8, dash=None, both=False):
        # trimmed to the circles it joins; dashed marks a kind carrying only
        # its owner's publications
        dx, dy = x2 - x1, y2 - y1
        ln = math.hypot(dx, dy) or 1.0
        ux, uy = dx / ln, dy / ln
        ax, ay = x1 + ux * t1, y1 + uy * t1
        bx, by = x2 - ux * t2, y2 - uy * t2
        b.append(line(ax, ay, bx, by, col, w, cap="round", dash=dash))
        b.append(head(ax, ay, bx, by, col))
        if both:
            b.append(head(bx, by, ax, ay, col))

    def card(x, y, m, kind, budget, failure, fail_col=quiet, links=(), teach=False):
        col = SERIES[m]
        b.append(rect(x, y, 236, 104, SURFACE, col if m == "M4" else RULE, 1.4))
        b.append(circle(x + 18, y + 16, 5, col))
        b.append(text(x + 30, y + 20, f"{m} · {kind}", 11.5, INK, weight="600"))
        b.append(text(x + 224, y + 37, budget, 10.5, INK, "end", "600"))
        ego = (x + 52, y + 60)
        peers = [(x + 124, y + 44), (x + 124, y + 60), (x + 124, y + 76)]
        for px_, py_ in peers:
            b.append(circle(px_, py_, 4.6, SURFACE, RULE, 1.5))
        b.append(circle(ego[0], ego[1], 8, SURFACE, INK_SOFT, 1.8))
        if teach:
            b.append(text(ego[0], y + 81, "one node", 8.5, quiet, "middle"))
        for (mode, label), (px_, py_) in zip(links, peers):
            if mode == "out":
                glyph_arrow(*ego, px_, py_, col, 11.0, 7.5)
            elif mode == "in":
                glyph_arrow(px_, py_, *ego, col, 7.5, 11.0)
            elif mode == "seed":
                glyph_arrow(*ego, px_, py_, col, 11.0, 7.5, w=1.2, dash="3 3")
            elif mode == "both":
                glyph_arrow(*ego, px_, py_, col, 12.0, 7.5, both=True)
            if label:
                b.append(text(x + 138, py_ + 3, label, 8.5, quiet))
        b.append(text(x + 118, y + 96, failure, 10, fail_col, "middle",
                      "600" if fail_col != quiet else None))

    band(38, 156, 1, "One link direction")
    card(60, 80, "M1", "push", "24 picks",
         "a node nobody drew cannot receive",
         links=[("out", None)] * 3, teach=True)
    card(564, 80, "M2", "pull", "24 picks",
         "a publisher nobody drew cannot be heard",
         links=[("in", None)] * 3, teach=True)
    # mirror images: the same primitive with the direction inverted
    b.append(line(316, 132, 544, 132, RULE, 1.4, cap="round"))
    b.append(head(544, 132, 316, 132, RULE))
    b.append(head(316, 132, 544, 132, RULE))
    b.append(text(430, 124, "the direction inverted", 10, quiet, "middle"))

    band(234, 156, 2, "Both directions, a second link kind")
    card(312, 276, "M5", "push + pull", "9 + 8 = 17 picks",
         "either failure, on one condition each",
         links=[("in", None), ("in", None), ("out", "everything")])
    card(564, 276, "M3", "pull + seeding", "13 + 6 = 19 picks",
         "either failure, on one condition each",
         links=[("in", None), ("in", None), ("seed", "own publications")])

    band(430, 156, 3, "Both directions, one link kind")
    card(312, 472, "M4", "symmetric", "9 picks",
         "cut off only if both fail at once", fail_col=ok,
         links=[("both", None)] * 3)
    b.append(text(566, 528, "the design the Specification fixes", 10, INK_SOFT))

    # the edges between the bands are Table 7's structural choices
    b.append(arrow(220, 194, 408, 230, RULE, 1.4))
    b.append(arrow(640, 194, 452, 230, RULE, 1.4))
    b.append(text(430, 216, "run at once", 10, quiet, "middle"))
    b.append(arrow(682, 194, 682, 230, RULE, 1.4))
    b.append(text(694, 216, "+ s − 1 seeding links", 10, quiet))
    b.append(arrow(430, 390, 430, 426, RULE, 1.4))
    b.append(text(442, 412, "the two link sets merged into one", 10, quiet))

    return frame(
        W, H, b, "The five dissemination designs",
        "Three numbered bands read downward, one card per design, each card "
        "showing one node and three of its peers. Band 1 holds the primitives, "
        "which cover one link direction each: under M1, push, a node forwards "
        "to targets it drew, and a node nobody drew cannot receive; under M2, "
        "pull, a node draws its forwarders, and a publisher nobody drew cannot "
        "be heard. Each meets the failure target at 24 picks. Band 2 covers "
        "both directions by adding a second link kind: M5 runs push and pull "
        "at once at 9 plus 8 picks; M3 keeps pull relays and adds seeding "
        "links carrying only their owner's publications, at 13 plus 6 picks. "
        "Either failure remains open, each resting on one condition. Band 3 "
        "merges the two sets into one bidirectional link kind: M4, the design "
        "the Specification fixes, reaches the same target at 9 picks and is "
        "cut off only when both conditions fail at once.",
        conditions="pick budgets at each design's proposed configuration: "
                   f"N = 20 000, μ = 0.2, δ = {decade(1e-4)}, "
                   "admission rules off")

# registered in main() after "derivation.svg":
#     "design-family.svg": fig_design_family(),


## ------------------------------------------------------------------ figure 5
def fig_evidence_map() -> str:
    """The Rationale's opening map: two layers of evidence, and what each fixes.

    Structural like Figures 1 and 3, so its content is literal rather than
    drawn from cells.json. The two pick counts it names are the proposal's own
    constants: RF = 9 is the configuration the ungated comparison carries for
    M4, RF = 10 the configuration the Specification fixes, both restated from
    Table 8 rather than measured here.

    Two bands, read downward, because that is the order the argument runs in:
    the first layer compares all five designs with the admission rules off, on
    the agreement of two independently built instruments, and selects one; the
    second sizes the admission rules on the selected design and arrives at the
    configuration a deployment actually runs. The bands are numbered so the
    prose can say which layer a number belongs to, which is the distinction
    readers trip on: the two layers name different configurations, and where
    they differ the gated numbers are the ones that apply.
    """
    W, H = 860, 480
    b = []
    quiet = "#8a887e"

    def band(y, h, n, title, note):
        b.append(rect(38, y, W - 76, h, SURFACE, GRID, 1.2, rx=8))
        b.append(circle(66, y + 19, 11, INK, INK, 0))
        b.append(text(66, y + 23, str(n), 11.5, SURFACE, "middle", "700"))
        b.append(text(84, y + 24, title, 12.5, INK, weight="600"))
        b.append(text(W - 52, y + 24, note, 10, quiet, "end"))

    def box(x, y, w, h, head, sub, stroke=RULE):
        b.append(rect(x, y, w, h, SURFACE, stroke, 1.4))
        b.append(text(x + w / 2, y + 20, head, 12, INK, "middle", "600"))
        b.append(text(x + w / 2, y + 36, sub, 9.5, quiet, "middle"))

    band(38, 196, 1, "What selects the design",
         "all five candidates · admission rules off")

    # the five candidates, one chip each in its own hue
    for i, m in enumerate(SERIES):
        x = 237 + i * 82
        b.append(rect(x, 78, 58, 26, SURFACE, SERIES[m], 1.6))
        b.append(text(x + 29, 95, m, 11.5, INK, "middle", "600"))
    b.append(text(430, 122, "one structural choice apart · Table 7", 10, quiet,
                  "middle"))

    for x in (253, 607):
        b.append(arrow(x, 130, x, 152, RULE, 1.6))
    b.append(text(430, 146, "each design measured by both", 10, quiet, "middle"))

    box(118, 156, 270, 48, "Analysis", "closed-form coverage laws")
    box(472, 156, 270, 48, "Measurement",
        "the node's own logic under a deterministic scheduler")

    # the two instruments face each other: their agreement is the argument
    b.append(arrow(392, 180, 424, 180, INK_SOFT, 1.6))
    b.append(arrow(468, 180, 436, 180, INK_SOFT, 1.6))
    b.append(text(430, 170, "agreement", 10, INK, "middle", "600"))
    b.append(text(430, 224, "built separately, failing in unrelated ways: "
                  "their agreement is the argument", 10, quiet, "middle"))

    # layer 1's outcome, carried between the bands
    b.append(arrow(430, 238, 430, 274, RULE, 1.6))
    b.append(text(442, 260, "selects M4, the symmetric relay link · "
                  "compared at RF = 9", 10, quiet))

    band(282, 170, 2, "What a deployment actually sees",
         "the selected design · gate and cap on")

    for x, head, sub in ((84, "Bucket count B", "how narrow the verifiable gate is"),
                         (325, "Serving cap C", "how many links a node accepts"),
                         (566, "Pick count k", "how many peers a node picks")):
        box(x, 322, 210, 48, head, sub)

    for x0, x1 in ((189, 350), (430, 430), (671, 510)):
        b.append(arrow(x0, 374, x1, 396, RULE, 1.4))

    box(230, 402, 400, 44, "M4 as specified · RF = 10, gated",
        "failure probability, links, copies and downtime budget: Table 8, last row",
        stroke=INK_SOFT)

    b.append(text(38, 468, "The two layers name different configurations: where "
                  "a design's numbers differ between them, the gated ones apply.",
                  11, INK_SOFT, style="italic"))

    return frame(W, H, b, "The evidence in two layers",
                 "Two numbered layers read downward. Layer 1 compares all five "
                 "candidate designs with the admission rules off: each design, "
                 "one structural choice apart from another, is measured by two "
                 "independently built instruments, analysis with closed-form "
                 "coverage laws and measurement running the node's own logic "
                 "under a deterministic scheduler, and their agreement is the "
                 "argument. It selects M4, the symmetric relay link, compared "
                 "at RF = 9. Layer 2 sizes the admission rules on the selected "
                 "design: the bucket count, the serving cap and the pick "
                 "count, arriving at the configuration specified, M4 at "
                 "RF = 10 under the gate and the cap, whose numbers are Table "
                 "8's last row. The two layers name different configurations, "
                 "and where a design's numbers differ the gated ones apply.",
                 conditions="N = 20 000 · μ = 0.2 · δ = 10⁻⁴")

# wiring in main(), alongside the other structural figures:
#     "evidence-map.svg": fig_evidence_map(),
...
def main() -> int:
    ...
    figs = {
        ...
        "design-family.svg": fig_design_family(),
        "evidence-layers.svg": fig_evidence_map(),
