...
1. **K sets the reliability floor.** The ungated law's own failure
   core is μ^K ("all my picks landed adversarial, nobody honest picked
   me"), and the gated form contains it at every B: as B → 1 the gated
   law recovers the ungated law exactly, from above. One pick ≈ one
   decade of floor. K is the only knob that buys reliability; it is
   paid in bandwidth (copies, standing links) — the CIP's economic
   axis.
...
refusals at 0.0025/victim (~81/run), admitted 5.997/5.997, Sybil
...
**The normalization**: attack surface per deposit-priced identity on
the relay seam. The pair draw's one coin covers both directions —
surface = (N−1)/B. The directional draw's two independent coins pay
twice — surface = 2(N−1)/B_r (measured: the E19 ordered arm's ≈ 2/B
admissibility). Equal surface therefore means B_r = 2·B_pair, which
runs the directional pools at **half** the pair draw's pool size.
...
- **At M4-equal attack surface, gated M3 is infeasible.** Surface 40
  forces B_r = 1000 → pools of 20; the deaf channel is pool-limited
  and no K repairs it (K = 9…13 all land at 2×10⁻³–10⁻² — raising K
  past the pool does nothing, lowering it re-opens μ^K). The pair draw
  gets pools of 40 at the same surface. This is the sharpest form of
  the comparison and it is a geometry statement, not a tuning gap.
...
Throughout: λ = (N−1)/B the pool mean, pool split h ~ Bin(H−1, 1/B),
a ~ Bin(S, 1/B), m = E[min(K, p)/p] the member-pick probability.
...
| isolation (two channels) | E_iso = H·Σ_{h,a} P(h)P(a)·(1−m)^h·[h = 0 or all-min(K,p)-picks-adversarial (hypergeometric)] | exact-arithmetic; channel A compactly ≈ H·e^{−(1−μ)λ} | E18 (B = 250/500, μ-axis); the K = 9 law first measured by the c-floor baseline (z = +0.81); every anchor here |
| pool-floor rule | (N−1)/B ≥ ln(H/δ)/(1−μ) | compact, from channel A | E18's design rule |
| pick-decade rule | one pick ≈ one decade of tail (μ^K core) | compact, first-order | §2's ladder; K = 9/10 rows measured |
...
| churn budget | P(bad) at μ_eff = (S + c·H)/N, solve for c | compact composition | reproduces the published ungated 7.43 %; §2's gated budgets |
...
| law | form | grade | measured |
|---|---|---|---|
| deaf channel | E_deaf = H·E_{h,a}[C(a, kp)/C(h+a, kp)], kp = min(K, h+a) | exact-arithmetic; compactly μ^K when pools ≫ K, **pool-limited when λ ≲ 2K** | E10 (r-ladder, K = 16); the cliff cell (17/400, all deaf, z = −0.06) |
...
ungated laws at the B = 1 limit — but no independent derivation with
stated assumptions and proofs exists in the formal folder's style. A
formal-team review of the gated forms (particularly the M4 two-channel
isolation and the M3 cross-seam products, which the CIP comparison now
leans on) is the natural hardening step and is listed in §11.
...
- **Formal-team review** of §9's gated forms (the hardening step for
  the CIP-facing claims).
