...
1. **Group all strategy policy under `strategies/`.** `src/strategies/` contains the three seam submodules (`connection`, `acceptance`, `fanout`) plus `config` (the ADR 0028 two-phase construction: per-seam `ConnectionParams`/`AcceptanceParams`, `StrategyConfigError`, `NodeStrategies`/`NodeStrategiesBuilder`). `strategy_config.rs` becomes `strategies::config`, dropping the prefix. `strategies::edge` is the home for the shared verifiable edge predicate (extracted in this feature once the acceptance seam became its second consumer — see the update note above and ADR 0030).
...
- Deferred: the shared edge-predicate extraction to `strategies::edge` lands with 015 (its first second consumer), avoiding a premature one-caller abstraction.

## Alternatives rejected

- **Leave the layout as-is** — keeps the loose `strategy_config.rs` / directory asymmetry and gives the shared edge predicate no natural home before 015 duplicates it.
...
- **Extract the edge predicate to `strategies::edge` now** — premature; a single caller until 015. Deferred to when the second consumer exists.
