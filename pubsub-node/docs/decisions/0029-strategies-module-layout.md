...
- Deferred: the shared `rank_key` extraction to `strategies::ranking` lands with 015 (its first second consumer), avoiding a premature one-caller abstraction.

## Alternatives rejected

- **Leave the layout as-is** — keeps the loose `strategy_config.rs` / directory asymmetry and gives the shared ranking helper no natural home before 015 duplicates it.
...
- **Extract `rank_key` to `strategies::ranking` now** — premature; a single caller until 015. Deferred to when the second consumer exists.
