...
2. **Interval carried in a read-only `NodeView` (not an ad-hoc param).** The recurring read-only node-state a strategy reads — `subscriptions`, `candidates`, `downstream`, and the current `interval` — is grouped into `strategies::view::NodeView<'a>` (borrows of the current `NodeState`). The seam signatures become `expected_upstream(&self, view: &NodeView)` and `admit(&self, emitter, topic, view: &NodeView)` — lean, and the "read-only context in → decision out" contract is explicit (`admit` went 6 params → 3). Per-decision params (`emitter`/`topic`) stay explicit; strategy config (`genesis`/`target_degree`/`cap_buffer`/`self_id`) stays on the strategy object. `NodeState` holds `interval: u64` (default 0), set by `Heartbeat`; `handle_connection_request` reads it via the view so the acceptor verifies against the current interval. The transition builds one `NodeView` per call (no copying). This is the shape the planned strategies-as-`apply`-arguments refactor generalises — introduced here, coordinated with that workstream so we do not define two context types. (Fan-out is unchanged: `targets` still takes `topic`/`downstream`/`exclude`.)
3. **Shared predicate module `strategies::edge`.** `is_valid_edge(genesis, topic, requester, candidate, interval, buckets) -> bool` and the small helpers `bucket_count(candidates_len, target_degree)` (`= max(1, round(len/target_degree))`) and `accept_cap(target_degree, c)` (`= ⌈target_degree + c·√target_degree⌉`) live here — the single source of the predicate/formulae the dial and accept seams both call. (This is the shared-helper home ADR 0029 anticipated; it is a *predicate*, not the earlier `rank_key`/PRNG sampler.)
