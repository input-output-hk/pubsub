...
10. **Maintain fanout with graceful degradation.** On disconnect, handshake failure, or descriptor-verification failure, resample from the candidate set to restore `d_target`. Re-read the subscription list periodically (cadence is a [configurable parameter](./README.md#configuration-parameters)) to track churn — joiners, leavers, topic-subscription changes, and endpoint updates all surface here. The node's chain follower is already running for relayer verification (see [README](./README.md#chain-access)), so this is not extra infrastructure.

    If `d_target` cannot be filled (genuinely small candidate set, peers temporarily unresolvable, transient churn), operate with whatever `d_actual ≤ d_target` is available — better one live peer than zero. Two thresholds gate alerting: log a warning when `d_actual < d_min_warn` and an error when `d_actual < d_min_error` (see [configuration parameters](./README.md#configuration-parameters)). Refill attempts run on exponential backoff capped at the list-re-read interval, so transient gaps recover automatically as new joiners or returning peers reappear.
