...
**Use Case Definition: Cardano PubSub for Cross-Chain "Bridge & Stake"**

## Executive Summary

This use case defines Cardano Cardano PubSub's role as the **"Interoperability Signaling Layer"** for the Partner Chain ecosystem (Midnight, Sidechains) and external networks.
...
Cardano PubSub collapses this into a **"One-Click Bridge & Stake"** experience. By decoupling the signaling of cross-chain events from the slow consensus of the underlying ledgers, Cardano PubSub allows users to express complex cross-chain intents (e.g., "Move my BTC from Bitcoin to Cardano and immediately stake it in Lending Protocol X") as a single message. Agents listen for these signals and execute the multi-chain orchestration.

## Strategic Value Proposition

| Value | Description |
|-------|-------------|
| **Liquidity Acquisition Funnel** | Transforms Cardano PubSub from a communication tool into a "TVL Magnet." Cardano DeFi protocols can proactively market yield opportunities to users on Partner Chains (e.g., Midnight) and offer immediate execution |
...
| **Reduced Latency Perception** | While the underlying bridge finality takes time, the Cardano PubSub signal is instant. The user receives immediate feedback ("Bridging in progress...") via the message bus, maintaining engagement |
| **Unified Standard** | Instead of every bridge building a proprietary notification system, Cardano PubSub provides a universal transport for "Bridge Events" and "Cross-Chain Intents" |

## Actors & Roles

| Actor | Role in Cardano PubSub | Description |
|-------|---------------|-------------|
| **Source User** | Publisher | The user on the external chain (e.g., Midnight, Bitcoin) initiating the transfer. They sign the Cardano PubSub Intent |
...
| **Bridge Agent** | Subscriber & Relayer | A specialized solver that listens for Cardano PubSub signals, verifies source-chain finality (proofs), and executes the mint/deposit on Cardano |
| **Verifier Node** | Relayer | Cardano PubSub nodes with the "State Verification" module enabled, capable of validating light-client proofs (Mithril, ZK) attached to messages |

## Operational Flow: "Midnight to Cardano Yield"

**Scenario:** Alice holds DUST on the Midnight blockchain. She receives an Cardano PubSub notification in her wallet: *"Earn 5% APY on your DUST by staking in Cardano Protocol X."*
...
- **Cardano PubSub Publication:** Once the lock is submitted, her wallet publishes a message to `crosschain/midnight-cardano/intents`
- **Payload:** Transaction Hash of the Lock + "Intent: Deposit to Protocol X"

### Step 3: Propagation & Proof Attachment

- **Observation:** The Cardano PubSub network propagates the intent
...
- **Update:** This proof is appended to the Cardano PubSub message thread or referenced in a follow-up "Proof Available" message

### Step 4: Destination Execution (The Agent)

- **Verification:** A Bridge Agent on Cardano receives the Intent + Proof via Cardano PubSub
...
- **Completion:** The Agent publishes a `execution/success` message to Alice's Cardano PubSub inbox
...
sequenceDiagram
    ...
    Alice->>Cardano PubSub: Publish cross-chain intent
    Midnight->>Cardano PubSub: Proof of finality
    Cardano PubSub->>Agent: Intent + Proof
    Agent->>Agent: Verify proof
    Agent->>Cardano: Mint + Deposit tx
    Cardano->>Alice: LP tokens received
    Agent->>Cardano PubSub: Success notification
    Cardano PubSub->>Alice: "Staking Complete"
```
...
message CrossChainIntent {
  ...
  // Optional: The cryptographic proof data (if small enough for Cardano PubSub msg)
  bytes proof_data = 6;
}
```
...
To support this, Cardano PubSub Nodes require a modular architecture where "Verification Plugins" can be loaded.
...
| Cross-Chain Requirement | Cardano PubSub Feature Support |
|------------------------|----------------------|
...
| **Proof Propagation** | DP-6 (Large Payloads): While Cardano PubSub is optimized for small messages, it must support "Blob Attachments" (up to ~64KB) for ZK-proofs, or reliable linking to IPFS/Arweave |
| **Agent Discovery** | ECO-4 (Service Discovery): Users need to find which Agents support "Midnight bridging." The Cardano PubSub Registry allows Agents to advertise their capabilities |
...
1. **Proof Size Limits:** ZK-SNARKs are small, but STARKs or full block headers might exceed Cardano PubSub's message size limits. Do we need a "Sidecar" download protocol for heavy proofs?
