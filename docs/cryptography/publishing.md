...
registers (and authenticate via some long-term key) fresh SUF or KES key, 
...
consists on an adversary coping all connections of some target peer `T`, thus
isolating `T` from the rest of the network (this is an _eclypse attack_.)
...
fix-structure messages_. This would be enough for the "IOG to SPOs" use case in 
...
Independently on whether the root of trust is stored in the topic registry, 
...
few) "main" root of trust -- e.g., the SPO cold key under which a opcert is 
...
just generate as many gossiping key pair as needed, until reaching one that
...
like in proof of stake, force the node operator to provide a receipt that some 
amount of ADA has been locked into a contract. Other similar options may be 
possible.
