...
This section specifies the protocol. It builds from the on-chain state upward: what the chain holds, what a node identity is, what an epoch and its randomness are, how a node derives the links it will hold, how those links are established, what a message is, and how messages travel and are recovered. What it aims at is an interoperable implementation written from this document alone.

It does not reach that standard yet, and where it falls short it says so at the point it falls short rather than in a list at the end. Three components are specified as **interfaces with stated requirements** instead of mechanisms, namely the [randomness beacon](#epochs-and-the-randomness-beacon), [address resolution](#address-resolution) and the on-chain validators, each of which a conforming deployment MAY satisfy in more than one way. One part of the design is **unresolved**: [the dissemination design](#the-dissemination-design) fixes which link kinds exist, how many links a node opens of each and in which direction they carry traffic, two candidates remain, the evidence measures both without selecting between them, and until the question closes an implementation MUST treat the choice as configuration. Six of the ten parameters in [Table 4](#table-4) carry a **rule or a bound rather than a value**. And the **transport is left to the deployment**: what is fixed here are the canonical byte strings every implementation must agree on, not the framing or session layer that carries them, subject to the rule that a peer's identity is taken from the signed preimage and never from the connection it arrived over.

What is settled throughout is the shape. Every mechanism is stated in terms of those rules and interfaces, so closing one of these questions supplies a value or a component without reinterpreting the design around it, and the [Versioning](#versioning) rules say how each such change reaches a running deployment.
...
A design's churn budget cannot be sampled directly. It is defined where *p*<sub>bad</sub> meets the 10⁻⁴ target, and resolving a rate that low takes on the order of 10⁵ to 10⁶ draws for every churn level tested. What can be tested is the reduction underneath it, the claim that downtime enters as a shift of the adversarial fraction, at parameters where failures are frequent enough to count. If that holds, the budgets follow from laws that Figure 3 has already validated.
