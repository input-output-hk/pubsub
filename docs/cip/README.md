...
> [!NOTE]
> **The exponents at a human scale.** Take a topic the size of today's stake-pool population, about 4,000 nodes, a fifth of them adversarial, and set the epoch to one day. Reading the [coverage law](#the-coverage-law) at that shape (*k* = 10, *B* = 128), the chance that a day's draw cuts off *some* honest node is about 2 × 10⁻⁶: one bad day expected in roughly 1,350 years. The chance that a *named* node is the one cut off is about 6 × 10⁻¹⁰ per day, one day in 1.6 billion, or once in about four million years. A node that finds itself silent should suspect its own connectivity or a correlated outage long before an unlucky draw.
