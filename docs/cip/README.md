...
>
> What holds the choice open is narrower than a tie. Two things would close it. The first is a fact about deployment rather than about the protocol: whether any operator expected to run this layer is genuinely bandwidth-bound *and* on few enough topics that connection count never binds. The second is an evidence gap that is ours to close — **the admission parameters are not measured for M4.** [Choosing the admission parameters](#choosing-the-admission-parameters) runs M2's relay wiring, and its sizing rules for the bucket count and the serving cap carry to M3 and M5 but explicitly not to M4, whose symmetric handshake changes the selection mechanism rather than sitting on top of it. Naming M4 today would name a design whose gate and cap have no measurement behind them. Both questions are posed in the Open Questions below, and the Specification names the design the answers select.
...
- Which of the two remaining designs to adopt. The measurements lean to M4, which leads four of the five coverage quantities, so what is open is narrower than a tie. Two things would close it. Whether any operator expected to run the layer is genuinely bandwidth-bound rather than connection-bound is a question about stake pools, wallet backends and dApp infrastructure, and needs evidence from them rather than further simulation. **The admission parameters, by contrast, are ours to measure:** the bucket count and serving cap are sized against M2's relay wiring and those rules do not carry to M4's symmetric handshake, so M4 cannot be named until that seam is run.
