...
The fork is a genuine trade and not a mistake on one side. M3 and M5 land at the same failure probability, 4.4 × 10⁻⁵, and at the same churn budget to within rounding, 2.17 % against 2.18 %. Specialising the second kind buys 24 % of the traffic; generalising it buys two picks' worth of state. Neither dominates, and both are still directional: each failure direction rests on a single condition, so the floor is *μ*<sup>*k*</sup> with nothing to rescue a node whose picks all failed.
