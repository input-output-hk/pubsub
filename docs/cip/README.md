...
**Rotation is argued, not measured.** Every draw counted in this section is a first draw. The instrument never advances the epoch: a run holds the topology derived at its genesis randomness from the first message to the last, which the experiments driver pins in a test named `epoch_nonce_stays_at_genesis_across_a_run`. That is coherent with what *p*<sub>bad</sub> is, a property of one drawn topology, and with churn being measured as absence within a draw rather than turnover between draws. What goes untested is the sequence rather than the draw: that successive draws are independent, which is the property [the beacon requirements](#epochs-and-the-randomness-beacon) exist to secure and which every argument from rotation rests on; that a subscriber [muted](#term-muting) in one epoch holds honest peers in the next; and that a retention floor of one epoch suffices for it to recover what it missed. Nothing measured here contradicts the three, and nothing measured here establishes them.
