...
- [Figure 5: The five dissemination designs](#figure-5)
- [Figure 6: The evidence in two layers](#figure-6)
- [Figure 7: Measured against predicted epoch failure probability](#figure-7)
- [Figure 8: Bandwidth, state and latency at the proposed configurations](#figure-8)
- [Figure 9: Four-way trade-off across the non-dominated designs](#figure-9)
- [Figure 10: The bucket count trade-off](#figure-10)
- [Figure 11: What each way of sizing the bucket count delivers, against topic size](#figure-11)
- [Figure 12: Measured configurations against the configuration proposed](#figure-12)
...
Five candidate designs for disseminating messages were analysed before one was chosen. The Specification fixes the symmetric relay link, which the analysis calls M4; [Designs evaluated](#designs-evaluated) sets out all five and the structural choice each one varies.

<div align="center">
<a name="figure-5" id="figure-5"></a>

![The five dissemination designs](images/design-family.svg)

<em>Figure 5: The five dissemination designs</em>

</div>

**The evidence comes in two layers.** The first selects the design: it compares all five on coverage and cost with the admission rules switched off. **Every comparison in this section is therefore ungated**, and the configurations it names are the ones the coverage models were evaluated at rather than the one this proposal specifies. Ungated is not a separate law: the gated coverage law reduces to the ungated one when the [gate](#the-verifiable-gate) is vacuous, the same family read at different bucket counts. The rules are left off for two reasons.

- **The gate costs the comparison almost nothing.** Coverage is unaffected wherever the gate leaves a node at least twice as many eligible peers as it must pick from, the [selection headroom](#term-r) the sizing rules hold at.
- **Leaving them off favours every design that lost rather than the one that won.** The two structural taxes the admission rules impose, set out under [Why the symmetric design](#why-the-symmetric-design), both fall on directional designs.

The second layer adds the verifiable gate and the serving cap, and gives the numbers a deployment would actually see. It is set out under [Selecting the design](#selecting-the-design), where the choice is settled, and [Sizing the parameters](#sizing-the-parameters), which establishes the rules the Specification states. Where a design's numbers differ between the layers, the gated ones apply.

<div align="center">
<a name="figure-6" id="figure-6"></a>

![The evidence in two layers](images/evidence-layers.svg)

<em>Figure 6: The evidence in two layers</em>
...
Five designs were analysed, each differing from another by one structural choice. [Figure 5](#figure-5) draws the family: the one-directional primitives, the two designs covering both failure directions with a second link kind, and the merge that covers both with one.
...
On cost, inversion buys nothing. M2 meets the same target at the same pick count, *RF* = 24, matches M1 on every cost axis to three figures, and [Figure 8](#figure-8) gives the two a single marker; the 0.2 hops M2 gains to its last subscriber is the whole of the difference. **Choosing between the primitives is a choice of which failure to suffer, not a cost decision, so the way out of twenty-four picks has to be structural.**
...
**M3 carries only its owner's own publications.** A node keeps M2's *RF* relay links and adds *s* − 1 standing initiation links, seeding each of its own messages to their intended initial holders rather than waiting to have been picked. The specialisation is what makes it cheap: a seeding link carries one node's traffic instead of the whole topic's, so the relay fanout can be smaller at the same coverage. At (*RF* = 13, *s* = 7) the budget is 19 picks against M2's 24, and traffic falls by 46 %, the cheapest in the field on bandwidth. State falls by much less, to 38 links, because 12 of them carry only their owner's publications: cheap to run, but still connection slots to provision and still exposed to churn.
...
The five designs nest, and the boundaries between them are checkable. M1 and M2 are the two halves of M5: switching off M5's inbound links leaves pure push, and switching off its outbound links pure pull. M3 is M2 at *s* = 1 by construction. M5 configured at those boundaries must therefore reproduce M1's and M2's results exactly, a free consistency check on both the analysis and the implementation: any discrepancy is a defect in one of the three rather than a property of the protocol.
...
<em>Figure 7: Measured against predicted epoch failure probability</em>
...
<em>Figure 8: Bandwidth, state and latency at the proposed configurations</em>
...
Those cells were chosen for measurability rather than realism, so the operating points themselves were then run at heavier downtime: 20, 25 and 30 % offline. **All nine placed the prediction inside the interval**, with a mean deviation of +0.30 and no detectable bias. A third round covered the two configurations this proposal names, which the second had run at superseded settings: M3 at (13, 7) and M4 at RF = 9, the latter at 35 % offline as well. **All six placed the prediction inside the interval**, M4 landing on its law almost exactly. Together the three rounds carry the reduction from an adversarial fraction of 0.20 out to 0.48.[^churn]
...
<em>Figure 9: Four-way trade-off across the non-dominated designs</em>
...
The published operating points were all chosen by one rule, the cheapest configuration meeting the failure target; searching each design's parameter space against the validated laws and then measuring the results shows what that rule costs. M3's re-split has already been described. The equivalent step for M4, from RF = 8 to RF = 9, buys **seven times the churn budget** (1.07 % to 7.43 %) for 1.6 further deliveries per node and two further connections. Only M3 and M4 were re-searched, being the two still in contention; M1, M2 and M5 remain at their cheapest-meeting-target points, the asymmetry the *p*<sub>bad</sub> column in Table 8 makes visible.
...
The churn axis is where the re-split does its visible work, though it does not change who leads: at M3's published split of (12, 8) that vertex is 0.54 % against M4's 7.43 %, a spike on bandwidth and very little else. Moving one link from seeding to relaying, at the same budget and the same links, quadruples it.

The re-split is not free on the axis M3 leads: the extra relay link is paid for in traffic, 10.4 deliveries per node against 9.6, which cuts M3's bandwidth lead over M4 at RF = 9 to 22 % from the 28 % the published split would show. M3 is thus plotted at its best *overall* split rather than its best *bandwidth* split, and the one axis it leads is drawn at its narrowest defensible margin. A reader weighing traffic against connections, the axis on which [the comparison](#why-the-symmetric-design) was long held open, should know that M3 has a further 6 % of bandwidth available, at the cost of three quarters of its churn tolerance and a longer path to the last subscriber.
...
**Weighting the axes does not decide this, and no longer needs to.** A weighting chooses among candidates that all clear the bar; it has nothing to say about one that does not. The question that long held the choice open, whether an operator's binding constraint is the traffic it carries or the connections it holds, was a question about bandwidth, the one axis the directional design leads; an answer favouring bandwidth would have bought a design that cannot reach the reliability target at equal attack cost. It remains a real question about deployment, worth answering; it is not this decision.
...
<em>Figure 10: The bucket count trade-off</em>
...
That boundary is what the [Specification's rule](#the-verifiable-gate) exists to respect. Figure 11 plots the failure probability each way of sizing arrives at, not the three bounds themselves: nearly proportional to the topic's size, they are parallel lines on log axes that a reader cannot separate.
...
<em>Figure 11: What each way of sizing the bucket count delivers, against topic size</em>
...
Figure 12 places the measured configurations and the proposed ones side by side: solid marks are configurations whose failure rate was counted; hollow marks are the configuration each design proposes, whose rate is a law prediction at a level no feasible sample can resolve; the dashed span between them is carried by the laws alone.
...
<em>Figure 12: Measured configurations against the configuration proposed</em>

</div>

The gap is close to two orders of magnitude for four of the five designs, and more than three for M4 at RF = 9, whose proposed point sits an order of magnitude inside the target rather than just under it. The laws are expected to be accurate across it, because the dominant failure mode in that range is the simplest one, a single node with no usable links, which they model exactly; [Figure 7](#figure-7) confirms they track measurement wherever measurement is possible. But the operating points themselves are predictions, not observations, and no amount of agreement at 10⁻² is a direct measurement at 10⁻⁴.
