...
The Cardano ecosystem lacks a decentralised layer for messages that do not belong on the chain itself. Governance discourse, dApp and wallet notifications, and oracle-style data feeds rely on centralised infrastructure today. Their operators can censor, fabricate, or silently drop messages. This undermines the trust guarantees of the underlying blockchain. Existing peer-to-peer solutions such as GossipSub do not close this gap. Their security rests on a discovery layer with freely created identities, which leaves them vulnerable to Sybil-based eclipse attacks.
