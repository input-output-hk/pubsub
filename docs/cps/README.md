...
Three things keep it from carrying the traffic described here.

- **Its membership is the wrong set.** No on-chain registration puts a wallet backend, a dApp service or a dRep into it, and those are how three of the four scenarios in this statement reach their audience.
- **Its guarantee does not transfer.** Chain diffusion is self-checking against withholding: blocks are expected at a known rate, so a node starved of them can tell and re-select its peers. A message that never arrives is indistinguishable from a message that was never sent.
- **It would share fate with what it protects.** Application-defined traffic on the links consensus runs over makes a flood in the messaging layer a consensus problem, and the channel that warns operators about a fault in their node would be running inside it.
