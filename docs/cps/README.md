...
Cardano already runs a peer-to-peer network, and it is the first candidate to rule out. Its node-to-node layer diffuses blocks and transactions between nodes, discovering peers from the relay addresses that stake pools register on chain together with whatever roots an operator configures by hand. It is engineered for that traffic, which is bounded, predictable, and has to propagate inside the time budget consensus assumes.

Three things keep it from carrying the traffic described here. Its membership is the wrong set: no on-chain registration puts a wallet backend, a dApp service or a dRep into it, and those are how three of the four scenarios in this statement reach their audience. Its guarantee does not transfer: chain diffusion is self-checking against withholding, since blocks are expected at a known rate, so a node starved of them can tell and re-select its peers, whereas a message that never arrives is indistinguishable from a message that was never sent. And it would share fate with what it protects: putting application-defined traffic on the links consensus runs over makes a flood in the messaging layer a consensus problem, and the channel that warns operators about a fault in their node would be running inside it.

Running a general-purpose gossip network beside the node avoids those three and fails at a different point. Mature gossip protocols, of which GossipSub is the widely deployed example, are engineered against message-level attacks such as flooding and spam, and mitigate them with peer scoring and mesh hardening.[^gossipsub] Their resistance to *eclipse* — a victim whose every neighbour is adversarial, and whose view of the network is therefore controlled — rests on the peer discovery layer beneath, and in the common libp2p deployment that layer admits freely created identities.[^libp2p] An adversary willing to run many of them can influence which peers a target connects to, and neither peer scoring nor mesh hardening restores a guarantee lost at the point of neighbour selection.
