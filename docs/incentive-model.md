...
The [AUEB research (D2)](D2-Cardano-PubSub-Framework-Design-and-Architecture.pdf) designed a three-layer dissemination protocol ([SecureCyclon](https://iohk.io/en/research/library/papers/securecyclon-dependable-peer-sampling/), [Vicinity](https://hal.science/hal-01480790/), Hybrid Dissemination) and a clique-DHT persistence layer with SPO-operated replication servers. It left the incentivization mechanism for replication servers as "work in progress." This document fills that gap. See also [research foundation](../site/architecture/research-foundation.md) and [protocol walkthrough](protocol-walkthrough.md) for how the three layers interact.
...
5. **Cross-topic storage and message ingestion:** Two sub-questions. First, topic selection: should servers store all topics assigned by consistent hashing (D2 Ch.4) or selectively choose profitable ones? Mandatory assignment is simpler but creates unfunded mandates. Second, ingestion path: replication servers don't strictly need to participate in the gossip protocol — publishers could send messages directly for storage. If servers run the 3-layer protocol they get messages "for free" but bear gossip overhead per topic, making selective hosting costlier. Direct push from publishers is simpler per-topic but adds a separate ingestion channel. The ingestion model affects whether selective vs mandatory hosting is practical.
